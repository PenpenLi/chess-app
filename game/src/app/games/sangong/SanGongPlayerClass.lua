local PokerClass = import(".SanGongPokerClass")
local C = class("SanGongPlayerClass",ViewBaseClass)
...
function C:onClickPoker( index )
    --播放点击牌音效
    PLAY_SOUND(GAME_SANGONG_SOUND_RES.."card_select.mp3")
...
function C:show( playerInfo )
    ...
    local scale = 130/self.accountLabel:getContentSize().width
    if scale > 1 then
        scale = 1
    elseif math.floor(scale*10) < 6 then
        scale = 0.6
    end
    self.accountLabel:setScale(scale)
    --位置
...
function C:showQiangTips( beishu, animation )
    ...
    local resname = GAME_SANGONG_IMAGES_RES.."grab_multi_"..beishu..".png"
...
function C:showBetTips( beishu, animation )
    ...
    self.tipsImg:setVisible(true)
    local resname = GAME_SANGONG_IMAGES_RES.."tag_"..beishu..".png"
    self.tipsImg:setTexture(resname)
    if animation then
        local endPos = cc.p(self.tipsImg:getPosition())
        local beginPos = cc.p(50,endPos.y)
...
function C:playBlinksAni( callback )
    --播放定庄音效
    PLAY_SOUND(GAME_SANGONG_SOUND_RES.."banker.mp3")
    self:showZhuang(true)
    --1
    local light1 = display.newSprite(GAME_SANGONG_IMAGES_RES.."box_light1.png")
    ...
    --2
    local light2 = display.newSprite(GAME_SANGONG_IMAGES_RES.."box_light2.png")
...
function C:sendPokerAni( delay, callback )
    ...
    local sendAni = function()
        --播放发牌音效
        PLAY_SOUND(GAME_SANGONG_SOUND_RES.."sendcard.mp3")
...
function C:showType( ntype,num )
    ...
    --播放摊牌音效
    PLAY_SOUND(GAME_SANGONG_SOUND_RES.."show.mp3")
    ...
    if ntype == 0 then
        ...
        self.niunBgImg:setTexture(GAME_SANGONG_IMAGES_RES.."cow_bg_black.png")
        self.niunNiuImg:setTexture(GAME_SANGONG_IMAGES_RES.."type/cow_0.png")

    --牛1-9
    elseif ntype == 1 or ntype == 2 then
        self.niunPanel:setVisible(true)
        self.niunBgImg:setTexture(GAME_SANGONG_IMAGES_RES.."cow_bg_yellow.png")
        self.niunNiuImg:setTexture(GAME_SANGONG_IMAGES_RES.."type/cow_"..num..".png")
        niuSp = self.niunNiuImg

    --牛牛
    elseif ntype == 3 then
        isNiuniu = true
        niuName = GAME_SANGONG_IMAGES_RES.."type/cow_10.png"

    --5花牛
    elseif ntype == 4 then
        isNiuniu = true
        niuName = GAME_SANGONG_IMAGES_RES.."type/cow_12.png"

    --炸弹牛
    elseif ntype == 5 then
        isNiuniu = true
        niuName = GAME_SANGONG_IMAGES_RES.."type/cow_11.png"

    --五小牛
    elseif ntype == em_BZNN_HandPX_FiveCalf then
        isNiuniu = true
        niuName = GAME_SANGONG_IMAGES_RES.."type/cow_13.png"
    end
    ...
    if niuSp then
        niuSp:setScale(3.5)
        niuSp:setOpacity(0)
        local particle = cc.ParticleSystemQuad:create(GAME_SANGONG_ANIMATION_RES.."particle/star01.plist")
...
function C:playTypeSound( ntype, num )
    ...
    PLAY_SOUND(GAME_SANGONG_SOUND_RES..name)
end
