...
function HBScene:onExitTransitionStart()
    self.m_pTextMyMoney:setString(utils:moneyString(dataManager.userInfo.money, 2))
	STOP_MUSIC()
...
dump(data, "新一轮")
    ...
    if math.abs(myWin) >= 1 then
        ...
        self.m_pTextMyWin:setString((myWin >= 0 and "+" or "") .. utils:moneyString(myWin, 2));
	    self.m_pTextMyMoney:setString(utils:moneyString(dataManager.userInfo.money, 2));
    end
...
--dump(data, "抢红包")
    ...
    if data.playerid == dataManager.userInfo.playerid then
	    self.m_arrPlayers[self.m_arrPlayers.curNum].get:setString(utils:moneyString(data.redMoney, 2));
        ...
        self.m_pRedPack:waitAndActions(0, cc.ScaleTo:create(0.2, 1.1), cc.CallFunc:create(function() 
            ...
            self.m_pRedPack.get:setString("+"..utils:moneyString(data.redMoney, 2))
...
print("结算")
	...
	for i = 1, self.m_arrPlayers.curNum do
        local get = utils:moneyString(self.m_arrPlayers[i].data.redMoney, 2)
        local win = utils:moneyString(self.m_arrPlayers[i].data.realMoney, 2)
