...
function C:initialize()
	...
	--自己头像
	local headRes =  GET_HEADID_RES(self.model.myInfo["headid"])
	self.myHeadImg:loadTexture(headRes)
	local money = utils:moneyString(self.model.myInfo["money"])
	self.myBlanceLabel:setString(money)
	local name = tostring(self.model.myInfo["playerid"])
...
function C:setUpBankerNeedMoney( money )
	local text = string.format("上庄需要%0.0f万",money/(10000*MONEY_SCALE))
...
function C:showStartAni()
	local strAnimName = GAME_BRNN_ANIMATION_RES.."skeleton/start/skeleton"
...
function C:showReward( emtype, totalMoney, myMoney )
    ...
    local skeletonNode = sp.SkeletonAnimation:create(strAnimName .. ".json", strAnimName .. ".atlas", 1)
    skeletonNode:setPosition(display.cx,display.cy+50)
...
function C:flyLuckyStarIfNeeded()
	if self.playerClassArr[1].info then
		...
		local flags = self.model:getHadFlyLuckyStar(2)
		if money > 0  and flags == false then
		...
		local flags = self.model:getHadFlyLuckyStar(3)
		if money > 0  and flags == false then
		...
		local flags = self.model:getHadFlyLuckyStar(4)
		if money > 0  and flags == false then
...
function C:onlinePlayerThrowChips( area,chips )
	self.chipsClass:throwManyChips(8,area,chips)
end
