...
function C:showServiceLayer()
	local url = tostring(DEFAULT_SERVICE_URL).."&info="..string.urlencode("userId=0".."&name=用户ID:"..""..",平台:"..tostring(dataManager.styleId)..",渠道:"..tostring(CHANNEL_ID).."&memo=0")
	SHOW_WEBVIEW(url)
end
...
function C:loadLoadingAnimation()
	local strAnimName ="base/animation/skeleton/loading/loading"
	...
	self.logo_ani:addChild( skeletonNode )
end
