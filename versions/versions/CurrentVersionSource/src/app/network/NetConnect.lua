...
function ConnectHallServer(callback)
    ...
    printInfo("连接大厅服务器:"..tostring(hallServerIp)..":"..tostring(hallServerPort));
end
...
function OnClientConnect()
    ...
    printInfo("连接大厅服务器成功:"..tostring(hallServerIp)..":"..tostring(hallServerPort));
end
...
function OnClientFaild()
    ...
    if reconnectHallServerCount > MAX_RECONNECT_COUNT then
        ...
        local keys = table.keys(dataManager.hallServers) or {}
        if hallServerIndex > #keys then
...
function OnClientClose()
    isHallConnecting = false
    HallServerConnected = false;
    printInfo("大厅服务器关闭:"..tostring(hallServerIp)..":"..tostring(hallServerPort));
    if not autoReconnectHallServer then
        return
    end
    if reconnectHallServerCount > MAX_RECONNECT_COUNT then
...
local function heartBeat()
    if utils then
        utils:createTimer("network.heartbeat",3,function()
            if HallServerConnected then
