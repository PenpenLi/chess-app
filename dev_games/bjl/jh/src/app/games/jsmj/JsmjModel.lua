...
function C:getWinSeat()
    return 1
end
function C:getSelfSeat()
    return 2
end
--获取本地座位
...
function C:addTileById(id)
    if id > 0 then
        table.insert(self.tiles_, Tile.new(id))
    end
end
...
function C:getTileIdByValue(value)
    local delIndex = -1
    for k, v in pairs(self.tiles_) do
        if self.tiles_[k].value_ == value then
            return self.tiles_[k].id_
        end
    end
end
...
function C:addOutTileById(seat, id)
    table.insert(self.outCard[seat], Tile.new(id))
end
