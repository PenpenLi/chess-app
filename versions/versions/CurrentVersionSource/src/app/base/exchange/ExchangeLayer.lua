...
C.RESOURCE_BINDING = {
    ...
    --银行卡选中背景
    yhk_tab_bgS = {path="box_img.yhk_tab_bgS"},
    --支付宝选中背景
    zfb_tab_bgS = {path="box_img.zfb_tab_bgS"},
    --支付宝wei选中背景
    zfb_tab_bgUn = {path="box_img.zfb_tab_bgUn"},
}
...
function C:onCreate()
    ...
    self.zfb_tab_bgS:setVisible(false)
    self.zfb_tab_bgUn:setVisible(false)
end
...
function C:createEditBox(handler)
	...
	local editBox = ccui.EditBox:create(cc.size(254,40),bg,bg,bg)
	editBox:setAnchorPoint(cc.p(0,0.5))
	editBox:setPosition(cc.p(220,121))
...
function C:onClickYhkTabBtn( event )
    ...
    self.yhk_tab_bgS:setVisible(true)
    self.zfb_tab_bgS:setVisible(false)
	...
	self.yhkTipsImg:setPosition(cc.p(0,16))
	...
	if dataManager.userInfo["BankAccountNum"] ~= nil and dataManager.userInfo["BankAccountNum"] ~= "" then
		...
		self.yhkAccountLabel:setTextColor(cc.c4b(255,255,255,255))
		self.yhkBindBtn:setVisible(false)
		self.yhkUpdateBtn:setVisible(true)
	else
		self.yhkAccountLabel:setString("未绑定银行卡账号")
		self.yhkAccountLabel:setTextColor(cc.c4b(218,219,220,255))
...
function C:onClickZfbTabBtn( event )
    ...
    self.yhk_tab_bgS:setVisible(false)
    self.zfb_tab_bgS:setVisible(true)
	...
	self.zfbTipsImg:setPosition(cc.p(0,16))
	...
	if dataManager.userInfo["zhifubao"] ~= nil and dataManager.userInfo["zhifubao"] ~= "" then
		...
		self.zfbAccountLabel:setTextColor(cc.c4b(255,255,255,255))
		self.zfbBindBtn:setVisible(false)
		self.zfbUpdateBtn:setVisible(true)
	else
		self.zfbAccountLabel:setString("未绑定支付宝账号")
		self.zfbAccountLabel:setTextColor(cc.c4b(218,219,220,255))
...
function C:yhkSetMoneyAndPercent( money, percent )
	...
	self.yhkTipsImg:setPosition(cc.p(x,self.yhkTipsImg:getPositionY()))
...
function C:onYhkEditHandler( event )
	...
	elseif event.name == "changed" then
		...
		if 0 < money and money*MONEY_SCALE > self.blanceCoin-10*MONEY_SCALE then
			...
			toastLayer:show("达到最大可兑换金额，兑换后账号上至少保留10金币")
		end
		if money < 0 then
			money = 0
		end
...
function C:onClickYhkExchangeBtn( event )
	...
	if money < 100 then
		toastLayer:show("兑换最小额度100金币，至少保留10金币")
		return
	end
...
function C:zfbSetMoneyAndPercent( money, percent )
	...
	self.zfbTipsImg:setPosition(cc.p(x,self.zfbTipsImg:getPositionY()))
