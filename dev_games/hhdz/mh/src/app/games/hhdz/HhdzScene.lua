...
local MAX_TREND_TYPE_NUM = 8

local TREND_LAYER_WIDTH = 484
local TREND_LAYER_HEIGHT = 65

local TREND_POINT_START_POSX = 18
local TREND_POINT_GAPX = 24
local TREND_POINT_POSY = 48

local TREND_TYPE_START_POSX = 38
local TREND_TYPE_GAPX = 60
local TREND_TYPE_POSY = 21
...
C.RESOURCE_BINDING = {
    ...
    --电池
    batteryNode={path="top_panel.bg_img.battery_node"},
	--帮助按钮
    ...
    --时间
    timeLabel={path="top_panel.bg_img.time_label"},
    --wifi
    wifiImg={path="top_panel.bg_img.wifi"},
    --4G
    G4Img={path="top_panel.bg_img.4g"},
    --king
    kingNode={path="top_panel.king"},
    --queen
    queenNode={path="top_panel.queen"},

    --下注区
    myBlackBetLabel = {path="bets_panel.my_black"},
    myRedBetLabel = {path="bets_panel.my_red"},
    myLuckyBetLabel = {path="bets_panel.my_lucky"},
    myBlackBetNode = {path="bets_panel.my_black"},
    myRedBetNode = {path="bets_panel.my_red"},
    myLuckyBetNode = {path="bets_panel.my_lucky"},
    allBlackBetLabel = {path="bets_panel.all_black"},
    allRedBetLabel = {path="bets_panel.all_red"},
    allLuckyBetLabel = {path="bets_panel.all_lucky"},
...
function C:loadResource()
    ...
    manager:addArmatureFileInfo(GAME_HHDZ_ANIMATION_RES .. "hongheidazhan_guowanghuanghou/hongheidazhan_guowanghuanghou.ExportJson")
	manager:addArmatureFileInfo(GAME_HHDZ_ANIMATION_RES .. "hongheidazhan_vsdonghua/hongheidazhan_vsdonghua.ExportJson")
    --加载plist图集
...
function C:unloadResource()
    local manager = ccs.ArmatureDataManager:getInstance()
    manager:removeArmatureFileInfo(GAME_HHDZ_ANIMATION_RES .. "hongheidazhan_guowanghuanghou/hongheidazhan_guowanghuanghou.ExportJson")
    manager:removeArmatureFileInfo(GAME_HHDZ_ANIMATION_RES .. "hongheidazhan_vsdonghua/hongheidazhan_vsdonghua.ExportJson")
    --卸载plist图集
...
function C:initialize()
    ...
    --适配宽屏
    self:adjustUI(nil,{self.backBtn},{self.topBg,self.helpBtn,self.settingsBtn})
    self.gameStartEffectNode:setPositionX(self.gameStartEffectNode:getPositionX()-self.offsetX)
    self.globalEffectNode:setPositionX(0)
    --屏蔽充值按钮
	...
	local armature1 = ccs.Armature:create("hongheidazhan_guowanghuanghou")
	armature1:getAnimation():play("Animation1")
    self.kingNode:addChild(armature1)
    armature1:setScale(0.75)
	armature1:setPosition(cc.p(0,0))

	local armature2 = ccs.Armature:create("hongheidazhan_guowanghuanghou")
	armature2:getAnimation():play("Animation2")
    self.queenNode:addChild(armature2)
    armature2:setScale(0.75)
...
--显示系统信息
function C:showSysInfo()
    self.timeLabel:setString(os.date("%H:%M"))
    local deviceNetWork = utils:getCurrentConnectType()
    if deviceNetWork == CONST_NET_TYPE_WIFI or CONST_NET_TYPE_UNKNOWN then
        self.wifiImg:setVisible(true)
        self.G4Img:setVisible(false)
    elseif deviceNetWork == CONST_NET_TYPE_4G then
        self.wifiImg:setVisible(false)
        self.G4Img:setVisible(true)
    end
...
function C:createTrend()
    ...
    self.trendLayer:setPosition(cc.p( 568 -TREND_LAYER_WIDTH / 2 -40,display.top - 194))
    ...
    if self.trendPoints and type(self.trendPoints) == "table" then
        ...
        if point then
            ...
            if lastPoint then
                ...
                point:pos(TREND_POINT_START_POSX,TREND_POINT_POSY)
            end
...
function C:throwChip(seat,betType,betIndex,isAnim,isDesk)
    ...
    self.chipsView:chipGo(seat,betIndex,randPos,isAnim,isDesk,isAnim and  self.playChipSound)
...
function C:playerVsAnim(callBack)
    ...
    local armature = ccs.Armature:create("hongheidazhan_vsdonghua")
	armature:getAnimation():play("Animation1")
	node:addChild(armature)
	armature:setPosition(cc.p(display.width/2,display.height/2))

    local nodeDelay = CCDelayTime:create(1)
    local nodeCallFun = CCCallFunc:create(function (  )
    	if callBack then
    		callBack()
    	end
    ...
    local nodeSeq = transition.sequence({nodeDelay,nodeCallFun})
