...
C.currentGame = nil
C.currentProgress = 0
C.currentUpdatingGameId = nil
C.gettingVersion = false
C.logining = false
C.gameRoomServerIndexs = {}
C.gameRoomServerLoginCount = {}
...
function C:registerAll()
	...
	Register(MainProto.RegLogin,RegLogin.SC_FULLCONNECT_ATTACK_P,handler(self,self.s2cServerFull))
...
function C:startUpdateGame(gameId)
    if self.updating then
        if gameId ~= self.currentUpdatingGameId then
            eventManager:publish("UpdateGameProgress",gameId,0)
        end
        self.updatingGames[tostring(gameId)] = true
        return
    end
    ...
    self.currentUpdatingGameId = gameId
    ...
    local function onError(errorCode)
        ...
        elseif errorCode == cc.ASSETSMANAGER_NETWORK then
            toastLayer:show(tostring(GAME_LIST[gameId]).."更新失败")
            eventManager:publish("UpdateGameFailed",gameId)
        end

        assetsManager:release()
        self.updatingGames[tostring(gameId)] = nil
        self.updating = false
        self.currentUpdatingGameId = nil
        local nextGame = tonumber(table.keys(self.updatingGames)[1])
...
function C:checkWaittingUpdateGames()
    local gameIds = table.keys(self.updatingGames)
    for k,v in pairs(gameIds) do
        if tonumber(v) ~= self.currentUpdatingGameId then
            eventManager:publish("UpdateGameProgress",tonumber(v),0 )
        end
    end
end
...
function C:enterGame(gameId,orderId,info,callback)
    ...
    self.currentOrderId = orderId    self.currentInfo = info
    toastLayer:hide()
...
function C:connectGameServer(gameId,orderId)
	local key = tostring(gameId).."_"..tostring(orderId)
	local index = self.gameRoomServerIndexs[key] or 0
    local info = dataManager:getGameServerInfo(gameId,orderId,index)
    if info then
...
--房间已满
function C:s2cServerFull(s)
    loadingLayer:show("房间爆满，正在切换服务器...")
	CloseGameServer()
	
	if self.currentGameId > 0 and self.currentOrderId > 0 then
		local key = tostring(self.currentGameId).."_"..tostring(self.currentOrderId)
		local count = self.gameRoomServerLoginCount[key] or 0
		local serverNum = dataManager:getGameServerNum(self.currentGameId,self.currentOrderId)
		if count < serverNum then
			local index = self.gameRoomServerIndexs[key] or 0
			self.gameRoomServerIndexs[key] = index + 1
			self.gameRoomServerLoginCount[key] = count + 1
			self:connectGameServer(self.currentGameId,self.currentOrderId)
		else
			self.gameRoomServerLoginCount[key] = 0
		end
	end
end
...
function C:forceQuitRoom(gameId,orderId,port)
    if (gameId == nil or gameId == self.currentGameId) and (orderId == nil or orderId == self.currentOrderId) and (port == nil or port == self.currentPort) and self.currentGame ~= nil then
