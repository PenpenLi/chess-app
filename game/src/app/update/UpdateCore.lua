...
local baseUpdateUrl = nil
local basePackageUrl = nil
local baseVersionUrl = nil
...
function C:startUpdateBase()
    ...
    if baseUpdateUrl ~= url then
        baseUpdateUrl = url
        --处理强更新功能解决问题时添加时间戳参数，保证更新连接hash不一样，更新里面对basePackageUrl增加后缀.zip判断，所以这里时间戳参数伪造.zip
        basePackageUrl = baseUpdateUrl.."package.zip".."?t="..tostring(os.time())..".zip"
        baseVersionUrl = baseUpdateUrl.."version".."?t="..tostring(os.time())
    end
