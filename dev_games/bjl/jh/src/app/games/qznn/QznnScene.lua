...
function C:initialize()
	...
	--绑定电池节点
	--self:bindBatteryNode(self.batteryBar,self.batteryLighting)已改为大厅控制
	self:bindBatteryNode(self.batteryNode)
	self:updateBattery()
	--加载plist图集，在CocosStudio里面没用过的不会自动加载
	display.loadSpriteFrames(GAME_QZNN_IMAGES_RES.."red_line.plist",GAME_QZNN_IMAGES_RES.."red_line.png")
...
function C:onExit()
	...
	--移除图集
	display.removeSpriteFrames(GAME_QZNN_IMAGES_RES.."card.plist",GAME_QZNN_IMAGES_RES.."card.png")
	display.removeSpriteFrames(GAME_QZNN_IMAGES_RES.."red_line.plist",GAME_QZNN_IMAGES_RES.."red_line.png")
...
function C:playChoiceZhuangAni( zhuangId, playerIds )
    ...
    utils:createTimer("qznn.ChoiceZhuangAni",1/60,function()
        ...
        if where == moveX then
			...
			self.maskPanel:removeAllChildren(true)
			self.maskPanel:setVisible(false)
