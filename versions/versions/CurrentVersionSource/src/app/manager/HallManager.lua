...
C.loginingPlayerid = nil
C.detectDisconnectTimes = 0
C.isNetworkDialogShowed = false
...
function C:registerAll()
	...
	Register(MainProto.Money,Money.SC_VIP_PAY_LIST_P,handler(self,self.s2cAgentList))
    ...
    eventManager:on("RequestAgentList",handler(self,self.c2sAgentList))

    --支付相关
    eventManager:on("Pay",handler(self,self.c2sPay))
    Register(MainProto.Money,Money.SC_PAY,handler(self,self.s2cPay))
    ...
    --活动任务
    eventManager:on("UpdateTaskInfoRequest",handler(self,self.c2sUpdateTaskInfo))
    Register(MainProto.Task,Task.SC_UPDATE_TASK_LIST,handler(self,self.s2cUpdateTaskInfo))
    eventManager:on("GetTaskRewardRequest",handler(self,self.c2sGetTaskRewardRequest))
    Register(MainProto.Task,Task.SC_GET_TASK_REWARD,handler(self,self.s2cGetTaskRewardResponse))
...
--游戏暂停
function C:onPause()
    print("============HallManager:onPause========")
    CloseHallServer()
end

--游戏恢复
function C:onResume()
    print("============HallManager:onResume========")
    ReconnetHallServer()
end

--网络连接失败弹窗
function C:showNetworkDialog()
    loadingLayer:hide()
    toastLayer:hide()
    CloseHallServer()
    CloseGameServer()
    ...
    function(ok)
        self.isNetworkDialogShowed = false
...
function C:EnterHall()
    ...
    utils:createTimer("DetectNetwork",3, function()
        --如果3次以上都没有收到大厅回应
        if (self.detectDisconnectTimes >= 3 or utils:isNetworkAvailable() == false ) and self.isNetworkDialogShowed == false then
            self.isNetworkDialogShowed = true
            self:showNetworkDialog()
        elseif utils:isNetworkAvailable() then
            if self.isNetworkDialogShowed then
                ConnectHallServer(handler(self,self.connectCallback))
            end
...
function C:connectCallback(r)
    if not r then
        if utils:isNetworkAvailable() then
            ConnectHallServer(handler(self,self.connectCallback))
        elseif self.isNetworkDialogShowed == false then
            self.isNetworkDialogShowed = true
            self:showNetworkDialog()
        end
    else
...
function C:quickLogin()
    printInfo("快速登陆")
    if self.loginingAccount then
        self:c2sLogin(self.loginingAccount,self.loginingPassword,self.loginingRandom,self.loginingWxcode,self.loginingSmscode,self.loginingPlayerid )
    else
        ...
        self:c2sLogin(account,password,random,"","",playerid)
    end
end
...
--发送登陆协议
function C:c2sLogin(account,password,random,wxcode,smscode,playerid)
    if not HallServerConnected then
		...
		self.loginingPlayerid = playerid
        ConnectHallServer(handler(self,self.connectCallback))
        return
    end
...
function C:s2cLogin(s)
	...
	s.vip = 12
	if s.vip then
		cc.UserDefault:getInstance():setIntegerForKey("VIP",s.vip)
	end
...
function C:refreshQrcodeUrl()
    ...
    local longUrl = homeUrl..utils:randomString(5,8)..".html?"..utils:randomString(3,5).."="..tostring(base64String).."&platform="..tostring(dataManager.styleId).."&id="..tostring(dataManager.playerId)
...
function C:createShortUrlBySina( longUrl, callback )
    local url = "https://api.uomg.com/api/long2dwz?dwzapi=urlcn&url="..tostring(longUrl)
...
--    end)
    utils:httpGet(url,function( response )
        local shortUrl = nil
        local tb = json.decode(LuaReomve(response,"\\"))
        shortUrl = tb["ae_url"]
--        if response and type(response) == "string" then
--            shortUrl = response
--        end
...
--请求代理列表
function C:c2sAgentList() 
	if (os.time()-dataManager.vipPayListLastSyncTime < dataManager.vipPayListSyncInterval) and dataManager.vipPayList and #dataManager.vipPayList > 0 then
		eventManager:publish("AgentList",dataManager.vipPayList)
		print("时间还没到")
		return
	end
	dataManager.vipPayListLastSyncTime = os.time()
	SendHallServer(MainProto.Money,Money.CS_VIP_PAY_LIST_P)  
end

function C:s2cAgentList( info )
    dump(info,"s2cAgentList")
    loadingLayer:hide()
    if info ~= nil then     --info and info.code == 0
--        local tb = json.decode(info.str)
--		if tb == nil or type(tb) ~= "table" then
--			return
--		end
--		dataManager.vipPayList = tb.data
        dataManager.vipPayList = info
		eventManager:publish("AgentList",info)
    end
end

--支付
function C:c2sPay(type, money,transdata)
    local min = 10
    ...
    elseif type == CONST_PAY_TYPE_VIP then
        min = dataManager.configs.MinBDPay
        max = dataManager.configs.MaxBDPay
    else
    ...
    local info = {}
    info.userid = dataManager.playerId
    info.amount = money
    info.paytype = type
    info.transdata = transdata
    dump(info,"请求支付接口")
    SendHallServer(MainProto.Money,Money.CS_PAY,info)
...
function C:s2cPay( info )
    dump(info,"s2cPay")
    loadingLayer:hide()
    if info and info.code == 0 and info.msg then
        if info.paytype ~= CONST_PAY_TYPE_VIP then
        utils:openUrl(tostring(info.msg))
        else
            toastLayer:show("申请提交成功，我们会尽快处理")
        end
    else
        local msg = "请求支付接口失败"
...
function C:s2cDelGameRoom(s)
    ...
    gameManager:forceQuitRoom(gameid,orderid,s.port)
    eventManager:publish("UpdateGameList")
    if gameManager.currentGameId == s.gameid and gameManager.currentPort == s.port then
...
--活动任务-更新活动任务列表（新增活动任务或者更新某个任务完成进度）
function C:c2sUpdateTaskInfo()
    --增加请求频率限制,20秒可以更新一次(一局游戏大概20秒)
    local lastUpdateTime = self.__lastUpdateTime or 0
    if os.time()-lastUpdateTime > 20 then
        SendHallServer(MainProto.Task,Task.CS_UPDATE_TASK_LIST)
    end
end

--活动任务-更新活动任务列表（新增活动任务或者更新某个任务完成进度）
function C:s2cUpdateTaskInfo(s)
    dump(s,"s2cUpdateTaskInfo")
    local tasklist = s
    if tasklist then
        self.__lastUpdateTime = os.time()
        local updateDataList = {}
        for i,v in ipairs(tasklist) do 
            --更新dataManager活动任务数据
            if v.tasktype == "liushui" then
                v.nowVal = v.nowVal / 100
                v.completeVal = v.completeVal / 100
            end
            dataManager:updateRenwu(v)
            table.insert(updateDataList,v)
        end
        --更新活动页面列表
        eventManager:publish("UpdateTaskInfo",updateDataList)
        --更新大厅活动红点
        self:updateHuoDongRedPoint()
    end
end

--活动任务-领取任务奖励
function C:c2sGetTaskRewardRequest(s)
    SendHallServer(MainProto.Task,Task.CS_GET_TASK_REWARD,s)
end

--活动任务-领取任务奖励结果
function C:s2cGetTaskRewardResponse(s)
    eventManager:publish("GetTaskRewardResponse",s)
    self:updateHuoDongRedPoint()
end

--活动任务-更新活动红点
function C:updateHuoDongRedPoint()
    local visible = false
    --获取任务列表
    local infoList = dataManager:getRenwuList()
    for i, info in pairs(infoList) do
        --判断是否有完成的活动任务，有的话显示红点（字段根据接口而定）
        if info.nowVal >= info.completeVal then
            visible = true
            break
        end
    end
    eventManager:publish("SetHuodongRedDot",visible)
end
