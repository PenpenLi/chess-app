...
--    SUB_C_EXIT_GAME = 3,
    createLines = {file = GAME_FRUIT_IMAGES_RES.."lines/xian_%d.png", pos = cc.p(670 + 150, 385)},
    ...
    sounds = {  
                bg =    GAME_FRUIT_SOUND_RES.."bgm.mp3",
                stop1 = GAME_FRUIT_SOUND_RES.."stop1.wav",
                stop2 = GAME_FRUIT_SOUND_RES.."stop2.wav",
                stop3 = GAME_FRUIT_SOUND_RES.."stop3.wav",
                stop4 = GAME_FRUIT_SOUND_RES.."stop4.wav",
                stop5 = GAME_FRUIT_SOUND_RES.."stop5.wav",
                acc1 = GAME_FRUIT_SOUND_RES.."expected.wav",
                acc2 = GAME_FRUIT_SOUND_RES.."expected.wav",
                acc3 = GAME_FRUIT_SOUND_RES.."expected.wav",
                acc4 = GAME_FRUIT_SOUND_RES.."expected.wav",
                acc5 = GAME_FRUIT_SOUND_RES.."expected.wav",
                }
}
...
--进入场景
function FruitScene:onEnterTransitionFinish()
	FruitScene.super.onEnterTransitionFinish(self)
    self:viewDidLoad()
end

--退出场景
function FruitScene:onExitTransitionStart()
	STOP_MUSIC()
    self:finalize()
	FruitScene.super.onExitTransitionStart(self)
end
...
function FruitScene:viewDidLoad()
    ...
    name:setPosition(cc.p(280 + 150, 708))
    money:setPosition(cc.p(1080 + 150, 708))
    name:setTextColor(cc.c3b(255, 222, 5))
    money:setTextColor(cc.c3b(255, 222, 5))
    local pBG = self:seekChild("img_main_bg")
    name:setName("Label_MyName")
    money:setName("Label_MyMoney")
    pBG:addChild(name)
    pBG:addChild(money)
	...
	pBackToGame:addClickEventListener(function()
		...
		self:CheckAutoStart();
        self.m_pLGWinNumCur:stopAllActions()
	end);
	...
	self.m_pMultipleBtnInc:addClickEventListener(function()
	    ...
	    self.m_pBetTotal:setString(utils:moneyString(self.m_nMaxBet / 5 * self.m_nCurMultiple))
	end)
	...
	self.m_pMaxBet:addClickEventListener(function()
		...
		self.m_pBetSingle:setString(utils:moneyString(self.m_nCurMultiple * self.m_llSingleBet));
	    self.m_pBetTotal:setString(utils:moneyString(self.m_nMaxBet))
	end)
	...
	self.m_pLittleGame:getParent():setContentSize(cc.size(1700, 750))
	self.m_pLittleGame:getParent():setPositionX(0)
    for _, v in pairs(self.m_pLittleGame:getParent():getChildren()) do
        v:setPositionX(v:getPositionX() + 160)
    end
	...
	pAmt:setPosition(cc.p(321 + 150, 659));
	self.m_pBG:addChild(pAmt);
	pAmt = utils.GetAmt("sgj_jmpaomad");
	pAmt:setPosition(cc.p(1010 + 150, 659));
	pAmt:setScaleX(-1);
	self.m_pBG:addChild(pAmt);
    ...
    local quit = ccui.Button:create(GAME_FRUIT_IMAGES_RES.."btn_back.png", GAME_FRUIT_IMAGES_RES.."btn_back.png")
    self.m_pBG:addChild(quit)
    quit:setPosition(cc.p((1634 * self:getScale() - display.width) / 2 / self:getScale() + 50, 710))
    quit:addClickEventListener(function() 
        self.core:quitGame()
    end)
    self.btHelp:setPositionX(quit:getPositionX() + display.width / self:getScale() - 100)
...
--    end
	...
	self.m_pBetSingle:setString(utils:moneyString(self.m_nCurMultiple * self.m_llSingleBet))
	self.m_pBetTotal:setString(utils:moneyString(self.m_nCurMultiple * self.m_llSingleBet * self.argT.lines))
...
function FruitScene:Update()
	...
	self.m_pMultipleBtnInc:setTouchEnabled(not self.m_bRunning and self.m_nTimesLeft4Free == 0)
	self.m_pMultipleBtnDec:setTouchEnabled(not self.m_bRunning and self.m_nTimesLeft4Free == 0)
...
function FruitScene:_show_win_panel(llWin, nLv)
	...
	pAmt:runAction(cc.Sequence:create(cc.DelayTime:create(0.5), cc.CallFunc:create(function()
        ...
        utils.numberGO(pMoney, 0, llWin, 2, function(a, over)
			pMoney:setString(utils:moneyString(a));
	        self.m_pIcome:setString(utils:moneyString(a));
			if (over) then
...
function FruitScene:_start_inner_game()
	PLAY_MUSIC(GAME_FRUIT_SOUND_RES.."bonus_bgm.mp3");
...
--				self.m_pPlayers:ActiveMe(false);
                ...
                dataManager.userInfo.money = self.m_LGameData.UserScore
                self.m_GameData.userScore = self.m_LGameData.UserScore
			end
			...
			if (nTimes == 0) then
				...
				self:_show_little_game_ui(false);
				self:CheckAutoStart();
	            PLAY_MUSIC(GAME_FRUIT_SOUND_RES.."bgm.mp3");
			end
		end)));
...
function FruitScene:OnNewFreeTimes(fResultLast)
	PLAY_MUSIC(GAME_FRUIT_SOUND_RES.."bgmFire.mp3")
