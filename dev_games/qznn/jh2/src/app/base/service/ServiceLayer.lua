...
function C:showWebView(fromShow)
	local doAction = function()
		...
		elseif self.webview == nil and (device.platform == "ios" or device.platform == "android")  then
	        ...
	        --客服URL
	        local url = tostring(DEFAULT_SERVICE_URL).."&info="..string.urlencode("userId=0".."&name=用户ID:"..tostring(dataManager.playerId)..",平台:"..tostring(dataManager.styleId)..",渠道:"..tostring(CHANNEL_ID).."&memo=0")
