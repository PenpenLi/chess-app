...
function C:createRequestParam(idx)
	...
	local vip = cc.UserDefault:getInstance():getIntegerForKey("VIP",0)
	if vip == 0 then
		vip = math.random(1,13)
	end
	sendStr = string.format(sendStr,vip,CHANNEL_ID);
    local strData = ""
    strData = "r=" .. os.time()
...
--        local sendStr = SERVERLIST[idx]
        ...
        local function onServerListResponse()
            ...
            if xmlHttpReq.readyState == 4 and (xmlHttpReq.status >= 200 and xmlHttpReq.status < 207) then
                dump(xmlHttpReq.response)

				if xmlHttpReq.response == nil or xmlHttpReq.response == "" then
					callback(false)
                    return
				end
                ...
                --新年开关
                if tb["isnewyear"] ~= nil and tb["isnewyear"] == 1 then
