...
C.ROOMS_CSB = {
    ...
    [GAMEID_HB] = {
        ...
        "base/rooms/HbRoom_2.csb",
        "base/rooms/HbRoom_3.csb",
        "base/rooms/HbRoom_4.csb",
    },
}
...
function C:initUI()
    ...
    --底分
    local difenStr = ""
    if self.roomInfo.gameid == GAMEID_HB then
        local arr = utils:stringSplit(tostring(self.roomInfo.name),"_")
        if arr and arr[2] then
            difenStr = tostring(arr[2])--string.gsub(tostring(arr[1]),"抢红包_","")
        end
    else
        local str = string.gsub(self.roomInfo.name, "底分", "")
        local num = tonumber(str)
        difenStr = string.format("底分:%0.0f元", num)
        if num < 1 then
            difenStr = "底分:"..tostring(num).."元"
        end
    ...
    if self.roomInfo.gameid == GAMEID_HB then
        local arr = utils:stringSplit(tostring(self.roomInfo.name),"_")
        if arr and arr[3] and arr[4] then
            text = tostring(arr[3]).."-"..tostring(arr[4])
        end
    else
        text = "入场:"..string.format("%0.0f元", self.roomInfo.money / MONEY_SCALE)
    end
