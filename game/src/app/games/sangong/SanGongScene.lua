local PlayerClass = import(".SanGongPlayerClass")
local ResultClass = import(".SanGongResultClass")
local TimerClass = import(".SanGongTimerClass")
local CalculateClass = import(".SanGongCalculateClass")
local PopupClass = import(".SanGongPopupClass")

local C = class("SanGongScene", GameSceneBase)
-- 资源名
C.RESOURCE_FILENAME = "games/sangong/SanGongScene.csb"
-- 资源绑定
C.RESOURCE_BINDING = {
...
function C:initialize()
	...
	self:setDifen(self.model.difen / MONEY_SCALE)
...
function C:onEnterTransitionFinish()
	...
	--播放背景音乐
	PLAY_MUSIC(GAME_SANGONG_SOUND_RES.."bg.mp3")
end
...
function C:loadResource()
    ...
    --加载plist图集
	display.loadSpriteFrames(GAME_SANGONG_IMAGES_RES.."poker.plist",GAME_SANGONG_IMAGES_RES.."poker.png")
end

--卸载资源
function C:unloadResource()
    --移除图集
	display.removeSpriteFrames(GAME_SANGONG_IMAGES_RES.."poker.plist",GAME_SANGONG_IMAGES_RES.."poker.png")
...
function C:playGameStartAni( callback )
	--播放开始游戏音效
	PLAY_SOUND(GAME_SANGONG_SOUND_RES.."game_start.mp3")
...
function C:playChoiceZhuangAni( zhuangId, playerIds )
    ...
    local box = display.newSprite(GAME_SANGONG_IMAGES_RES.."choice_box.png")
    ...
    utils:createTimer("qznn.ChoiceZhuangAni",1/60,function()
        ...
        if not isGo[where] then
            isGo[where] = true 
            --播放音效
            PLAY_SOUND(GAME_SANGONG_SOUND_RES.."choosing.mp3")
        end
