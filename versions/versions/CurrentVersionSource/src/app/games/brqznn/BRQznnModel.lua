...
function C:reset()
	...
	self.quitPlayerIds = {}
	self.enterPlayerIds = {}
	utils:removeTimer(self.timerName..".playTypeSound")
end

--特殊处理牌类型的音效
local isplaying = nil
function C:playTypeSound(name)
	if isplaying then
		printInfo(">>>>>>>>>有声音在播放，不播放此声音>>>>>>>"..name)
		return
	end
	isplaying=true
	PLAY_SOUND(name)
	utils:delayInvoke(self.timerName..".playTypeSound",0.8,function()
		isplaying=false
	end)
end
