...
function C:onCreate( event )
    ...
    local label = self.accountEditBox:getChildByTag(10000)
    label:setPosition(cc.p(72,28))
    self.accountBg:addChild(self.accountEditBox)

    self.codeEditBox = self:createEditBox("输入验证码",cc.EDITBOX_INPUT_MODE_NUMERIC)
    self.codeEditBox:setContentSize(cc.size(240,70))
    local label = self.codeEditBox:getChildByTag(10000)
    label:setContentSize(cc.size(240,70))
    label:setPosition(cc.p(62,28))
...
function C:createEditBox( placeholder, inputMode )
    ...
    local editBox = ccui.EditBox:create(cc.size(415,70),bg,bg,bg)
    editBox:setAnchorPoint(cc.p(0,0.5))
    editBox:setPosition(cc.p(0,35))
    ...
    label:setTag(10000)
    label:setContentSize(cc.size(415,70))
    label:setPosition(cc.p(160,28))
