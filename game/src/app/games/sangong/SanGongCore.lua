local C = class("SanGongCore",GameCoreBase)

--模块路径
C.MODULE_PATH = "app.games.sangong"
--场景配置
C.SCENE_CONFIG = {scenename = "sangong_scene", filename = "SanGongScene", logic="SanGongLogic", define="SanGongDefine", model="SanGongModel"}

function C:start()
	self:registerGameMsg(self.define.SC_SG_CONFIG_P,handler(self,self.onConfig))
	self:registerGameMsg(self.define.SC_SG_GAMESTATE_P,handler(self,self.onGameState))
	self:registerGameMsg(self.define.SC_SG_CARD,handler(self,self.onSendCard))
	self:registerGameMsg(self.define.SC_SG_PLAYER_INFO,handler(self,self.onPlayerInfo))
	self:registerGameMsg(self.define.SC_SG_SHOW_JIESHUAN,handler(self,self.onSettlement))
	self:registerGameMsg(self.define.SC_SG_SHOWCARD,handler(self,self.onShowCard))
	C.super.start(self)
end

function C:exit()
	self:unregisterGameMsg(self.define.SC_SG_CONFIG_P)
	self:unregisterGameMsg(self.define.SC_SG_GAMESTATE_P)
	self:unregisterGameMsg(self.define.SC_SG_CARD)
	self:unregisterGameMsg(self.define.SC_SG_PLAYER_INFO)
	self:unregisterGameMsg(self.define.SC_SG_SHOW_JIESHUAN)
	self:unregisterGameMsg(self.define.SC_SG_SHOWCARD)
	C.super.exit(self)
end
...
function C:updatePlayerMoney( info )
    ...
    utils:delayInvoke("showqznncoin"..playerId,3.5,function ()
	...
	elseif self.model.currentGameState == self.define.GAMESTATE_FAPAI or self.model.currentGameState == self.define.GAMESTATE_TANPAI then
		...
		for i=1,5 do
			if info[i] then
				...
				if isGaming then
					--闲家已选择下注倍数
					if info[i]["playerid"] ~= m_zhuangPlayerId then
						self.scene:showPlayerBetTips( localSeatId, info[i]["peilv"], false )
					end
					--显示牌
...
function C:onGameState( info )
	...
	elseif self.model.currentGameState == self.define.GAMESTATE_XIUXI then
		...
		self.scene:showTimer(self.model.currentLeftTime,SanGongTimerClassTypeGameStart,function( finished,time )
	...
	elseif self.model.currentGameState == self.define.GAMESTATE_QIANGZHUANG then
		...
		local showQiangTimer = function( time )
			...
			if self.model.playerGameStateArr[1] and self.model.hadQiang == false then
				...
				self.scene:showTimer(time,SanGongTimerClassTypeQiangZhuang,callback)
			else
				self.scene:showTimer(time,SanGongTimerClassTypeQiangZhuang2,callback)
			end
		end
	...
	elseif self.model.currentGameState == self.define.GAMESTATE_TANPAI then
		...
		if self.model.playerGameStateArr[1] and self.model.hadTan == false then
			self.scene:showTanpaiBtn()
			self.scene:showCalculatePanel()
			self.scene:showTimer(self.model.currentLeftTime,SanGongTimerClassTypeTanpai,callback)
		else
			self.scene:showTimer(self.model.currentLeftTime,SanGongTimerClassTypeTanpai2,callback)
		end
