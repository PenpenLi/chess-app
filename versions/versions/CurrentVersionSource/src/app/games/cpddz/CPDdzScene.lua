...
--显示加倍
function C:showJiaBei(seat,jiabei,silent)
    self.players[seat]:showJiaBei(jiabei,silent)
end
...
--出牌
function C:showCards(seat,cards,anim,showAll,isFinish)
    printInfo(">>>>>>>>出牌>>>>>>>>"..seat)
    printInfo(isFinish)
    self.players[seat]:showCards(cards,anim,showAll,isFinish)
end
...
function C:showResult(isLord,isWin,isSpring)
    ...
    local sound=GET_MUSIC_VOLUME()
    if sound>=10 then
        SET_MUSIC_VOLUME(10,false)
        self:delayInvoke(sound_delay,function()
            SET_MUSIC_VOLUME(sound,false)
        end)
    end  
    ...
    local winArr = {}
    winArr[#winArr+1] = CCScaleTo:create(0.2, 1.1)
    winArr[#winArr+1] = CCScaleTo:create(0.2, 1)
    winArr[#winArr+1] = CCDelayTime:create(0.5)
    if isSpring then
        winArr[#winArr+1] = CCCallFunc:create(function()
            self:playSpringAnim()
        end)
        winArr[#winArr+1] = CCDelayTime:create(2)
    else
        winArr[#winArr+1] = CCDelayTime:create(1)
    end
    winArr[#winArr+1] = CCCallFunc:create(function()
        sp:setVisible(false)
        self.resultNode:setVisible(false)
    end)
	local winAction = transition.sequence(winArr)

    local loseArr = {}
    loseArr[#loseArr+1] = CCMoveTo:create(0.2, cc.p(pos.x, pos.y -10))
    loseArr[#loseArr+1] = CCMoveTo:create(0.1, cc.p(pos.x, pos.y))
    loseArr[#loseArr+1] = CCDelayTime:create(0.5)
    if isSpring then
        loseArr[#loseArr+1] = CCCallFunc:create(function()
            self:playSpringAnim()
        end)
        loseArr[#loseArr+1] = CCDelayTime:create(2)
    else
        loseArr[#loseArr+1] = CCDelayTime:create(1)
    end
    loseArr[#loseArr+1] = CCCallFunc:create(function()
        sp:setVisible(false)
        self.resultNode:setVisible(false)
    end)
	local loseAction = transition.sequence(loseArr)

    sp:stopAllActions()
	sp:runAction(isWin and winAction or loseAction)

    return (isWin and 2.4 or 2.3) + (isSpring and 2 or 0)
end
...
--        self:handleSelAction(self.touchCards)
...
function C:handleSelAction(touchCards)
    local mpID2Card = {}
    local selNew = false
    local upCardsAndSel = {}
    local touchCards2 = {}
	...
	for i, id in pairs(self.model.myCards) do 
		local rect, card = self:getCardRect(id);
        if card:isUp() then
            upCardsAndSel[card] = card.id
        end
        if touchCards2[card] then
            upCardsAndSel[card] = card.id
        end
        mpID2Card[card.id] = card
	end

    local hintCardsSets = {}
    local isCardsIn = function(cardSet, cards)
        for card, _ in pairs(cards) do
            if not cardSet[card] then
                return false
            end
        end
        return true
    end
    local getSetBySubCards = function(cardSets, cards)
        local allCardsIn = {}
        for i, v in ipairs(cardSets) do
            if isCardsIn(v, cards) then
		        table.insert(allCardsIn, i)
            end
        end
        return allCardsIn
    end
    if TableLen(self.logic.otherCards) > 0 then
        if not self.logic.allHints then self.logic:getAllHints() end
        for i, v in ipairs(self.logic.allHints) do
	        local keys = {};
            local hint = self.logic:cardsToProto(v)
	        for i,id in pairs(hint) do 
                keys[mpID2Card[id]] = id
	        end 
		    table.insert(hintCardsSets, keys)
        end
    end

    local upCards = {}
    local downCards = false

    if selNew then
        if tcNum == 1 then
            local setNOs = getSetBySubCards(hintCardsSets, touchCards2)
            if #setNOs == 0 then
                -- 上选中
                upCards = touchCards2
            else
                -- 上选中所在hint组全部 下其他
                upCards = hintCardsSets[setNOs[1]]
                downCards = true
            end
        else
            local setNOs = getSetBySubCards(hintCardsSets, upCardsAndSel)
            upCards = touchCards2
            if #setNOs == 0 then
                local selCards = {}
                for k, v in pairs(upCardsAndSel) do
                    table.insert(selCards, k.id)
                end
                tcAndSelNum = TableLen(upCardsAndSel)
                local cardsBackup = self.logic.cards
                self.logic.cards = self.logic:protoToCards(upCardsAndSel)
                local cards = self.logic:getSingleTriple()
                if not cards or TableLen(self.logic:cardsToProto(cards)) ~= tcAndSelNum then
                    cards = self.logic:getSelectedStraight()
                    if not cards or TableLen(cards) ~= tcAndSelNum then
                        -- 上选中下其他
                        selCards = {}
                        for k, v in pairs(touchCards2) do
                            table.insert(selCards, k.id)
                        end
                        self.logic.cards = self.logic:protoToCards(selCards)
                        cards = self.logic:getSelectedStraight()
                        if cards then
                            cards = self.logic:cardsToProto(cards)
                            upCards = {}
	                        for i,id in pairs(cards) do 
		                        local card = self.myCardView:getChildByTag(id);
                                upCards[card] = card.id
	                        end  
                        end
                        downCards = true
                    end
                end
                self.logic.cards = cardsBackup
            end
        end
    else
        -- 下掉选中
        downCards = touchCards2
    end

    -- action
	for i, id in pairs(self.model.myCards) do 
		local rect, card = self:getCardRect(id);
        if upCards[card] then
            if not card:isUp() then
                card:upCard()
            end
        else
            if downCards == true then
                card:downCard()
            elseif downCards == false then
            elseif downCards[card] then
                if card:isUp() then
                    card:downCard()
                end
            end
...
function C:showSelectedHintCards(touchCards)
	...
	elseif #self.touchCards > 2 then
		utils:delayInvoke("ddz.selected.hint.cards",0.1,
            function()
                local cards = self.logic:getSelectedHintCardsForFirst(self.logic:protoToCards(self.model.selectedCards))
                --dump(cards,"选牌2")
		        -- 有牌才会改变，如果是nil，就什么都不做
		        if cards then
		            self:showTipCards(self.logic:cardsToProto(cards))
...
function C:resetShowedSelectedHintCards()
	-- 自己回合并且是在连选2张牌以上才生效
	if self.model.isMyTurn and #self.model.selectedCards > 0 then
        utils:delayInvoke("ddz.reset.selected.hint.cards",0.1,
            function() 
                local cards = self.logic:getSelectedHintCards(self.logic:protoToCards(self.model.selectedCards))
                --dump(cards,"选牌1")
		        -- 有牌才会改变，如果是nil，就什么都不做
...
function C:turnBackCards(callBack)
    ...
    self.players[2]:turnBackCards()
    self.players[3]:turnBackCards()
end
...
function C:playExchangeSpringAnim(isClockWise,callBack)
    ...
    armature:setPosition(self.effectNode:convertToNodeSpace(cc.p(display.cx,display.cy + 20)))
...
function C:turnFrontCards(cards,callBack)
    ...
    self.players[2]:turnFrontCards()
    self.players[3]:turnFrontCards()
end
...
function C:showSelectBlindCards(cards,advanceCard,dizhuSeat,timeOut,isAnim)
    ...
    local offeset2 = GET_PHONE_HAIRE_WIDTH()

    printInfo("__________offeset2________________" .. tostring(offeset2))
    ...
    elseif dizhuSeat == 3 then
        toPos =cc.p( -293 - offsetX + offeset2 ,500)
        toScale = 0.7
...
function C:adjustHeadUI(leftItems,rightItems)
    ...
    if leftItems then
        for i,v in ipairs(leftItems) do
            v:setPositionX(v:getPositionX() - offsetX + GET_PHONE_HAIRE_WIDTH())
        end
    end
    if rightItems then
