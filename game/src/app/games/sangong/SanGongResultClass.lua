local C = class("SanGongResultClass",ViewBaseClass)
...
function C:playWinnerAnimation( seatId )
	local playerNode = self.playerNodeArr[seatId]
	local frame = cc.ParticleSystemQuad:create(GAME_SANGONG_ANIMATION_RES.."particle/frame.plist")
    frame:setAutoRemoveOnFinish(true)
    frame:setAnchorPoint(cc.p(0.5, 0.5))
    frame:setPosition(cc.p(50,50))
    playerNode:addChild(frame,-1)

    local star = cc.ParticleSystemQuad:create(GAME_SANGONG_ANIMATION_RES.."particle/star.plist")
...
function C:showYouWin( callback )
    --播放赢音效
    PLAY_SOUND(GAME_SANGONG_SOUND_RES.."game_win.mp3")
	self.node:setVisible(true)
...
function C:showYouLose( callback )
    --播放输音效
    PLAY_SOUND(GAME_SANGONG_SOUND_RES.."game_over.mp3")
	self.node:setVisible(true)
...
function C:flyGold( parent, startPos, endPos, callback, tag )
    local gold = display.newSprite(GAME_SANGONG_IMAGES_RES.."fly_gold.png")
    gold:setScale(0.7)
    gold:setTag(tag)
...
function C:playFLyGoldSound()
    ...
    utils:createTimer("qznn.QznnFlyGoldTimer",0.2,function()
        --播放飞金币音效
        PLAY_SOUND(GAME_SANGONG_SOUND_RES.."fly_gold.mp3")
    end)
