...
function C:showDetailInfo()
    ...
    local localPos = cc.p(_localPos.x + 250, _localPos.y )
    if self.seat == SEAT_RIGHT then
        localPos = cc.p(_localPos.x - 250, _localPos.y)
    elseif self.seat == SEAT_MID then
        --localPos=cc.p(172,195)
        local myNodePos = cc.p(self.headImg:getPosition())
        local myNodeWorldPos = self.headImg:getParent():convertToWorldSpace(myNodePos)
        local _localPos = self.globalEffectNode:convertToNodeSpace(myNodeWorldPos)
        localPos =cc.p(_localPos.x+105, _localPos.y+150)
    end
...
function C:showClock(time,callback,pos)
    ...
    self.clockBgImg:setVisible(true)
    self.clockLabel:setVisible(true)
...
--出牌
function C:showCards(cards,anim,showAll,isFinish)
...
function C:playBombAni()
    ...
    local strAnimName = GAME_CPDDZ_ANIMATION_RES.."sanim/boom/bomb"
...
function C:playRocketAni()
    ...
    local strAnimName = GAME_CPDDZ_ANIMATION_RES.."sanim/rock/rocket"
    local skeletonNode = sp.SkeletonAnimation:create(strAnimName .. ".json", strAnimName .. ".atlas", 1)
    skeletonNode:addTo(self.globalEffectNode,99)
    local p = self.globalEffectNode:convertToNodeSpace(CENTER)
	skeletonNode:setPosition(p)
...
function C:showExchangeOutCards(cards)
    ...
    local originPos = cc.p(self.selectCardNodelBirthPos.x,self.selectCardNodelBirthPos.y)
    self.selectCardsNode:setPosition(originPos)
...
function C:turnBackCards(callBack)
    ...
    if self.seat == SEAT_LEFT or self.seat == SEAT_RIGHT then
        ...
        if self.seat == SEAT_LEFT then
            originPos.x = originPos.x + offsetX - offset2
        elseif self.seat == SEAT_RIGHT then
            originPos.x = originPos.x - offsetX + offset2
        end
        local pos = cc.p(self.selectCardsNode:getPosition())
        if math.floor(pos.x - originPos.x) ~= 0 or math.floor(pos.y - originPos.y) ~= 0 then
            table.insert(arrayAction2,CCDelayTime:create(0.5))
            table.insert(arrayAction2,CCEaseSineOut:create(CCMoveTo:create(0.5,originPos)))
        ...
        table.insert(arrayAction2,CCDelayTime:create(0.6))
...
function C:turnFrontCards(cards,callBack)
    ...
    if self.seat == SEAT_LEFT or self.seat == SEAT_RIGHT then
        -- local pos = cc.p(self.selectCardsNode:getPosition())
        -- if math.floor(pos.x - self.selectCardNodelBirthPos.x) ~= 0 or math.floor(pos.y - self.selectCardNodelBirthPos.y) ~= 0 then
        --     table.insert(arrayAction2,CCEaseSineOut:create(CCMoveTo:create(0.3,self.selectCardNodelBirthPos)))
        -- end
    else
        self.selectCardsNode:setOpacity(255)
        self.selectCardsNode:setRotation(0)
        ...
        cardView:setPosition(cc.p(x - 2 ,y + 2))  -- 微调的结果 - 1 , + 2
...
function C:playDiscardedAmin(callBack)
    ...
    printInfo("_________playDiscardedAmin_______")
