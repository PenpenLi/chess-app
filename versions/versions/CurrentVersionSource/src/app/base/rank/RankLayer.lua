...
C.RESOURCE_BINDING = {
    ...
    today_win_tab_s = {path="box_img.today_win_tab_s"},
    today_time_tab_s = {path="box_img.today_time_tab_s"},
}
...
function C:onCreate()
	...
	--设置ID
	self:setMineIdLabel(self.todayWinMineImg)
	self:setMineIdLabel(self.todayTimeMineImg)
	self:setMineIdLabel(self.weekWinMineImg)
	self:setMineIdLabel(self.rewardMineImg)
...
function C:showTabPanel( index )
    ...
    self.today_win_tab_s:setVisible(index==1)
	self.todayWinTabBtn:setEnabled(index~=1)
	self.todayTimePanel:setVisible(index==2)
    self.today_time_tab_s:setVisible(index==2)
...
function C:setMineIdLabel( mineImg )
