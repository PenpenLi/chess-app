...
local CHIP_ANIM_MAX_NUM = device.platform == "android" and 30 or 40
...
local STAR_POS = 
{
    {x = display.cx + 172.5, y = display.cy + 125},
    {x = display.cx -175.5, y = display.cy + 125},
    {x = display.cx + 0.8, y = display.cy - 50}
}
...
local PLAYERS_POS = 
{
    [1] = {x = 70 + (IS_IPHONEX and 80 or 0), y = display.cy + 165},
    [2] = {x = 70 + (IS_IPHONEX and 80 or 0), y = display.cy - 25},
    [3] = {x = 70 + (IS_IPHONEX and 80 or 0), y = display.cy - 145},
    [4] = {x = display.right - 70 + (IS_IPHONEX and -80 or 0), y = display.cy -25},
    [5] = {x = display.right - 70 + (IS_IPHONEX and -80 or 0), y = display.cy -145},
    [6] = {x = display.right - 70 + (IS_IPHONEX and -80 or 0), y = display.cy + 165},
    [7] = {x = 70 + (IS_IPHONEX and 130 or 0), y = 50},
    [8] = {x = display.right - 45 + (IS_IPHONEX and -130 or 0) , y = 45}
}
...
C.BINDING = 
{
    blackAreaCon = {path="black_area_con",events={{event="click",method="onBlackBet"}}},
    redAreaCon = {path="red_area_con",events={{event="click",method="onRedBet"}}},
    luckyAreaCon = {path="lucky_area_con",events={{event="click",method="onLuckyBet"}}},
}
...
--筹码飞往下注区
function C:chipGo(seatIndex,betIndex, pos,isAnim,isDesk,playSound)
    ...
    chip:setPosition(PLAYERS_POS[seatIndex])

	local speed = 2500
	local time = cc.pGetDistance(cc.p(chip:getPositionX(), chip:getPositionY()), pos) / speed
	local move = CCMoveTo:create(time,cc.p(pos.x, pos.y))
	local scale1 = CCScaleTo:create(time, 1.1)
	local spawn = transition.spawn({move,scale1})
	local scale2 = CCScaleTo:create(0.4, 1)
	local seq = transition.sequence({spawn,scale2})

	transition.execute(chip, seq, 
    {
		easing = "OUT",
    	onComplete = function()
    		self.curChipAnimNum = self.curChipAnimNum - 1
