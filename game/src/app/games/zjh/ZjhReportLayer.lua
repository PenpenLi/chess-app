...
function C:onCreate()
	...
	local text = "被举报人："..tostring(self.info["playerid"])
	if self.info["nickname"] ~= nil and self.info["nickname"] ~= "" and tostring(self.info["nickname"]) ~= tostring(self.info["playerid"]) then
		text = text..tostring(self.info["nickname"])
	end
