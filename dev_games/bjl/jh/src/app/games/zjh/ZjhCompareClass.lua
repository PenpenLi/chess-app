...
function C:playCompareAni( winner, loser, isBoy )
	self:setVisible(true)
	...
	local callfunAttack2 = CCCallFunc:create(function ()
    	...
    	local name = "fail"
		if isBoy then
			name = string.format(name.."_boy_%d.mp3",index)
		else
			name = string.format(name.."_girl_%d.mp3",index)
		end
		PLAY_SOUND(GAME_ZJH_SOUND_RES..name)
	end)
