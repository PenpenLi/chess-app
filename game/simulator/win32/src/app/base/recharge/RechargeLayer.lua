...
C.RESOURCE_BINDING = {
	...
	--flash面板账号ID文本
	flashAccountLabel = {path="box_img.flash_panel.bottom_panel.account_img.label"},
	--flash面板复制ID按钮
	flashCopyBtn = {path="box_img.flash_panel.bottom_panel.copy_btn",events={{event="click",method="onClickFlashCopyBtn"}}},
...
function C:initFlashPanel()
	self.flashAccountLabel:setString(tostring(dataManager.playerId))
	self:loadFlashInfo()
end
...
function C:createFlashItem( index, info)
	...
	nameLabel:setString(tostring(info.AgentName))

	local ratingLabel = item:getChildByName("rating_label")
	local rating = 5.0--math.random(48,50)/10
	ratingLabel:setString(string.format("%0.1f",rating))

	local ypjLabel = item:getChildByName("ypj_label")
	local ypj = 200--math.random(10,50)*10
	ypjLabel:setString(string.format("月评价 %d+",ypj))
...
function C:showFlashLoading()
	...
	self.flashLoadingLabel:setString("正在获取代充列表，请稍后...")
	local array = {}
