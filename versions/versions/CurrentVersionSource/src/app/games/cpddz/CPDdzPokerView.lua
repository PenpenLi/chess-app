local C = class("CPCard",cc.Node)
local Model = import(".CPDdzModel")
...
function C:ctor(params)
	...
	self.isExchangeCard = params.isExchangeCard
	self.up = false;
	self.isSelect = false;
	self.sound = false;
	self.isFinish=params.isFinish
	self.animVector = {};
	...
	if self.isFinish and self.lastCard  then
		printInfo(">>>>>>>>>>>>>isFinish>>>>>>>>>>>>>>>")
		local lordIcon = display.newSprite(GAME_CPDDZ_IMAGES_RES.."lord_win_flag.png");
		--lordIcon:align(display.RIGHT_TOP, bg:getContentSize().width - 2, 2);
		lordIcon:setAnchorPoint(cc.p(1, 1))
		lordIcon:setPosition(cc.p(bg:getContentSize().width,bg:getContentSize().height))
		lordIcon:addTo(bg);
   end  
	...
	if params.isBlind or self.isExchangeCard then
	...
	elseif id == BLACK_JOKER then 
		...
		if not isBottom then
			if not self.playing or self.lastCard then
				...
				self:showAnmition(suitBig,false,true);
			end 
		else 
...
function C:showAnmition(target, isBg,isSuit)
	if not isBg then 
	...
	if self.showIdx >= 0 and self.anim then 
		...
		local spwan = transition.spawn({
   			CCFadeIn:create(t),
   			CCScaleTo:create(t, isSuit and 0.8 or 1.0),
		...
		local seq = transition.sequence({
			...
			CCCallFunc:create(function ()
				if not self.sound then 
					self.sound = true;
					-- printInfo(">>>>>>>>s_sendcard>>>>>>>>>>>>")
					-- PLAY_SOUND(GAME_CPDDZ_SOUND_RES.."s_sendcard.mp3")
					Model:playSendCardSound()
				end 
			end)
   		});
...
function C:getSuitImage(suit, num)
	-- 屏蔽JQK的Q版人物，换成原来的简单属性
	if suit == SUIT.SPADE then 
		return --[[num > 10 and "#card_spade"..tostring(num) or]] "#card_spade";
	elseif  suit ==  SUIT.HEART then 
		return --[[num > 10 and "#card_heart"..tostring(num) or]] "#card_heart";
	elseif  suit ==  SUIT.DIAMOND then 
		return --[[num > 10 and "#card_diamond"..tostring(num) or]] "#card_diamond";
	elseif  suit ==  SUIT.CLUB then 
		return --[[num > 10 and "#card_club"..tostring(num) or]] "#card_club";
	end
	return "";
end 
...
-- 地主没有选择的底牌
function C:setDiscardsFlag(isDiscard)
	if self.discard then
		self.discard:setVisible(isDiscard)
	else
		self.discard = display.newSprite(GAME_CPDDZ_IMAGES_RES .. "hide_bg.png")
		self.bg:addChild(self.discard)
		local size = self.bg:getContentSize()
		self.discard:setContentSize(size)
		self.discard:setPosition(cc.p(size.width / 2,size.height / 2 + 3))

		local flag = display.newSprite(GAME_CPDDZ_IMAGES_RES .."bblord_infobar_discarded_flag.png")
		self.discard:addChild(flag)
		flag:setPosition(cc.p(50,20))
		self.discard:setVisible(isDiscard)
	end
end
