...
C.RESOURCE_BINDING = {
	...
	--底分文本
	difenLabel = {path="top_panel.difen_label"},
	--帮助按钮
	...
	--摊牌按钮
	tanpaiBtn = {path="tanpai_btn",events={{event="touch",method="onClickTanpaiBtn"}}},
	--计算牌类型页面
...
function C:ctor(core)
	...
	for i=1,5 do
		...
		local key1 = string.format("playerBtn%d",i)
		local path1 = string.format("player_%d.head_panel.info_img",i)
		self.RESOURCE_BINDING[key1] = {path=path1,events={{event="click",method="onClickPlayerPanel"}}}
	end
	--抢庄按钮
	for i=1,4 do
		local key = string.format("qiangBtn%d",i)
		local path = string.format("qiang_panel.btn_%d",i)
		self.RESOURCE_BINDING[key] = {path=path,events={{event="touch",method="onClickQiangBtn"}}}
	end
	--下注按钮
	for i=1,4 do
		local key = string.format("betBtn%d",i)
		local path = string.format("bet_panel.btn_%d",i)
		self.RESOURCE_BINDING[key] = {path=path,events={{event="touch",method="onClickBetBtn"}}}
	end
...
function C:initialize()
	...
	for i=1,5 do
		...
		if i==1 then
			player:setCalculateCallback(function( num1,num2,num3 )
				local niu= self.calculateClass:setNumber(num1,num2,num3)
				if niu then
					self.canshow=true
					self.tanpaiBtn:loadTextureNormal(GAME_QZNN_IMAGES_RES.."action_youniu.png")
				else
					self.canshow=false
					self.tanpaiBtn:loadTextureNormal(GAME_QZNN_IMAGES_RES.."action_meiniu.png")
				end
			end)
		end
	end
	...
	local strAnimName =GAME_QZNN_ANIMATION_RES.."skeleton/kaishi/kaishi"
	self.kaishiAni = sp.SkeletonAnimation:create(strAnimName .. ".json", strAnimName .. ".atlas", 1)
	self.kaishiAni:setVisible(false)
	local offsetX = (display.width-1136)/2
	self.kaishiAni:setPosition(cc.p(display.cx-offsetX,display.cy))
	self.popupPanel:addChild( self.kaishiAni )
	--zorder 6
	self.maskPanel:setLocalZOrder(self.MASK_ZORDER)
	self.maskPanel:setVisible(false)
	self.choicePanel:setLocalZOrder(self.MASK_ZORDER+2)
	...
	local function touchBegin(touch, event)
		...
		local strAnimName =GAME_QZNN_ANIMATION_RES.."skeleton/dianji/dianji"
		...
		-- local sprite=display.newSprite("common/images/close_btn.png")
		skeletonNode:setPosition(cc.p(location))
		-- self:addChild(sprite)
		local array={}
		array[1]=CCDelayTime:create(0.6)
    ...
    local eventDispatcher = self.popupPanel:getEventDispatcher()
    eventDispatcher:addEventListenerWithSceneGraphPriority(listener, self.popupPanel)
end
...
function C:onEnterTransitionFinish()
	...
	SET_ROLL_ANNOUNCE_PARENT_POSY(self,610)
end
...
function C:onExitTransitionStart()
	...
	utils:removeTimer("qznn.settlement")
	utils:removeTimer("qznn.settlement1")
	utils:removeTimer("qznn.alertrecharge")
...
-- 		DialogLayer.new(true):show("是否确认本局结算后，自动离开",function(isOk)
-- 			self.model.autoQuit = isOk
-- 		end)
...
function C:onClickTanpaiBtn( event )
	if event.name == "ended" then
		...
		if self.hasNiu and not self.canshow then
			toastLayer:show("有牛的哦，请再努力一下")
			return
		end
		...
		--self:hideTimer()
		self.timerClass:onChangeState(QznnTimerClassTypeTanpai3)
...
function C:onClickQiangBtn( event )
	if event.name == "ended" then
		...
		self:hideQiangPanel()
		--self:hideTimer()
		self.timerClass:onChangeState(QznnTimerClassTypeQiangZhuang3)
	end
	-- local beishu = 0
	-- if index == 1 then
	-- 	beishu = 1
...
function C:onClickBetBtn( event )
	if event.name == "ended" then
		...
		self:hideBetPanel()
		--self:hideTimer()
		self.timerClass:onChangeState(QznnTimerClassTypeXiaZhu2)
	end
	-- local beishu = 5
	-- if index == 2 then
	-- 	beishu = 10
...
function C:playGameStartAni( callback )
	...
	self.kaishiAni:setVisible(true)
	self.kaishiAni:setAnimation(0,"animation",false)
...
function C:playChoiceZhuangAni( zhuangId, playerIds )
	local zhuangSeat=self:getLocalSeatIdByPlayerId(zhuangId)
	if #playerIds == 1 then
		local arr = {}
	...
	local aa = 0.8/(#playerIds*(1/60)*12)
	aa=math.min(aa,6)
	...
	for i = 1, #playerIds do
		...
		array[#array+1]=CCDelayTime:create(1/60*12)
	end
...
--隐藏下注倍数
function C:hidePlayerBetTips( localSeatId )
	local playerClass = self.playerClassArr[localSeatId]
	playerClass:hideBetTips()
end
...
function C:sendPlayerPokerAni( localSeatId, delay )
	...
	if localSeatId == 1 then
		playerClass:sendPokerAni(delay,function()
			-- utils:delayInvoke("qznn.sendpoker",0.04,function()
...
function C:doSettlement( info )
	local callback = function()
		...
		utils:delayInvoke("qznn.settlement1",0.4,function()
			for i=1,5 do
				self:hidePlayerQiangTips(i)
				self:hidePlayerBetTips(i)
			end
		end)
	end
	utils:delayInvoke("qznn.settlement",0.5,function()
		if callback then
			callback()
		end
	end)
		...
		if info[i] then
			...
			if playerId ~= self.model.zhuangId then
				...
				if localSeatId > 0 then
					local temp = {}
					temp.localSeatId=localSeatId
					temp.changemoney=changemoney
					if changemoney <= 0 then
						table.insert(loseLocalSeatIds,temp)
					else
						table.insert(winLocalSeatIds,temp)
					end
				end
			end
		end
...
function C:playChangeMoneyAni( info )
	for i=1,5 do
		if info[i] then
			...
			--玩家金币不足
			if playerId == dataManager.userInfo["playerid"] then
				if self.model.isKicked or self.model.inmoney > dataManager.userInfo.money then
