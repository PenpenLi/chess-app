...
C.RESOURCE_BINDING = {
	...
	node_headBg = {path="top_panel.head_node.node_headBg"},
	...
	idLabel = {path="top_panel.head_node.label"},
    ...
    node_gameHallLogo={path="top_panel.node_gameHallLogo"},
	--bottom panel
	...
	leftPanel = {path="center_panel.left_panel"},       --.scrollview
	pageview = {path="center_panel.left_panel.pageview",events={{event="event",method="onEventPageView"}}},     --.scrollview
	pageDot1 = {path="center_panel.left_panel.dot_1"},
	pageDot2 = {path="center_panel.left_panel.dot_2"},
    pageDot3 = {path="center_panel.left_panel.dot_3"},
	rightPanel = {path="center_panel.scrollview.right_panel"},
	gameItem = {path="item"},
	pageItem = {path="page_1"},
	pageItem2 = {path="page_2"},
    pageItem3 = {path="page_3"},
...
function C:initialize()
    ...
    self.pageItem3:setVisible(false)
    ...
    --eventManager:publish("RefreshQrcodeUrl")
	--进入大厅更新活动列表
    ...
    self:loadGameHallLogoAnimation()
...
function C:adjustUI()
	...
	self.centerScrollview:setContentSize(cc.size(display.width -323,self.centerScrollview:getContentSize().height))
...
function C:OnADClick_1()        --广告01
    --self:onClickGameItem(GAMEID_QZNN)
end

function C:OnADClick_2()        --广告02
    --self:onClickGameItem(GAMEID_JSMJ)
end

function C:OnADClick_3()        --广告03
    if dataManager:getGameLocalVersion(GAMEID_ZJH) < dataManager:getGameRemoteVersion(GAMEID_ZJH) then
        toastLayer:show("当前游戏有更新，请右侧点击游戏图标进行更新！！",2)
    else
        self:onClickGameItem(GAMEID_ZJH)
    end
end
...
--	self:addPage(2,6)
    ...
    self.pageDot3:loadTexture("base/images/main_layer/dot2.png")
	self:startPageTimer()
end
...
function C:onTouchPageview( event )
	...
	elseif event.name == "cancelled" then
		if 1 == self.pageview:getCurrentPageIndex() then
			self.pageview:scrollToPage(1,0)
		elseif 2 == self.pageview:getCurrentPageIndex() then
			self.pageview:scrollToPage(2,1)
        elseif 3 == self.pageview:getCurrentPageIndex() then
			self.pageview:scrollToPage(3,2)
		end
		self:startPageTimer()
...
--			self.pageview:scrollToPage(3,0)
--		end
		if self.pageview:getCurrentPageIndex() == 0 then
			self.pageDot1:loadTexture("base/images/main_layer/dot1.png")
			self.pageDot2:loadTexture("base/images/main_layer/dot2.png")
            self.pageDot3:loadTexture("base/images/main_layer/dot2.png")
		elseif self.pageview:getCurrentPageIndex() == 1 then
			self.pageDot1:loadTexture("base/images/main_layer/dot2.png")
			self.pageDot2:loadTexture("base/images/main_layer/dot1.png")
            self.pageDot3:loadTexture("base/images/main_layer/dot2.png")
        elseif self.pageview:getCurrentPageIndex() == 2 then
            self.pageDot1:loadTexture("base/images/main_layer/dot2.png")
			self.pageDot2:loadTexture("base/images/main_layer/dot2.png")
            self.pageDot3:loadTexture("base/images/main_layer/dot1.png")
		end
	end
end
...
function C:onClickGameItem( gameId )
    --print("gameId--------********" .. gameId)  toastLayer:show("歡迎進入炸金花!")
...
function C:showRoomLayer( gameId, items, animation )
	self.gameId = gameId
    --print("gameId---------------***********************" .. gameId)
    if(gameId == GAMEID_ZJH) then     --炸金花
        self:zjhRoomIn()
    elseif(gameId == GAMEID_QZNN) then    --抢庄牛牛
        self:qznnRoomIn()
    elseif(gameId == GAMEID_JSMJ) then    --极速麻将
        self:jsmjRoomIn()
    elseif(gameId == GAMEID_BRQZNN) then    --看牌抢庄牛牛
        self:brqznnRoomIn()
    elseif(gameId == GAMEID_FISH) then    --极速捕鱼
        self:fishRoomIn()
    elseif(gameId == GAMEID_DDZ) then    --斗地主
        self:ddzRoomIn()
    elseif(gameId == GAMEID_CPDDZ) then    --百变斗地主
        self:cpddzRoomIn()
    elseif(gameId == GAMEID_BRNN) then    --百人牛牛
        self:brnnRoomIn()
    elseif(gameId == GAMEID_FRUIT) then    --水果机
        self:fruitRoomIn()
    elseif(gameId == GAMEID_HB) then    --红包扫雷
		self:hbRoomIn()
	elseif(gameId == GAMEID_QZPJ) then    --抢庄牌九
        self:qzpjRoomIn()
    elseif(gameId == GAMEID_SANGONG) then    --三公
        self:sangongRoomIn()
    else
...
function C:hallOutAni()
	...
	self.rightPanel:setPosition(self.hairOffsetX,0)     --323+
...
function C:hallInAni()
	...
	self.rightPanel:setVisible(true)
	self.rightPanel:runAction( cc.MoveTo:create(0.25,cc.p(self.hairOffsetX,0)) )        --323+
	--bottom panel
	self.bottomPanel:stopAllActions()
...
function C:setAccount( account )
    --print("id  content----------**********" .. tostring(account))
	self.idLabel:setString("ID: " .. dataManager.userInfo.playerid)        --tostring(account)
	...
	local posX = self.headNode:getPositionX() + 42  --+38+width+40
	self.blanceNode:setPositionX(posX)
end

--设置金币
function C:setBlance( blance )
	local string = utils:moneyString(blance,1)
...
--进入炸金花
function C:zjhRoomIn()
    self.core:showZJHRoomLayer()
end

--进入抢庄牛牛
function C:qznnRoomIn()
    self.core:showQZNNRoomLayer()
end

--进入极速麻将
function C:jsmjRoomIn()
    self.core:showJSMJRoomLayer()
end

--进入看牌抢庄牛牛
function C:brqznnRoomIn()
    self.core:showBRQZNNRoomLayer()
end

--进入极速捕鱼
function C:fishRoomIn()
    self.core:showFishRoomLayer()
end

--进入斗地主
function C:ddzRoomIn()
    self.core:showDDZRoomLayer()
end

--进入百变斗地主
function C:cpddzRoomIn()
    self.core:showCPDDZRoomLayer()
end

--进入百人牛牛
function C:brnnRoomIn()
    self.core:showBRNNRoomLayer()
end

--进入水果机
function C:fruitRoomIn()
    self.core:showFruitRoomLayer()
end

--进入红包扫雷
function C:hbRoomIn()
    self.core:showHBRoomLayer()
end

--进入抢庄牌九
function C:qzpjRoomIn()
    self.core:showQZPJRoomLayer()
end

--进入三公
function C:sangongRoomIn()
    self.core:showSanGongRoomLayer()
end--大厅logo动画
function C:loadGameHallLogoAnimation()
	local strAnimName ="base/animation/skeleton/hall/gamehall_logo_ske"
    local skeletonNode = sp.SkeletonAnimation:create(strAnimName .. ".json", strAnimName .. ".atlas", 1)
    skeletonNode:setAnimation(0,"animation",true)
	self.node_gameHallLogo:addChild( skeletonNode )

    self.node_gameHallLogo:setScaleX(0.88)
    self.node_gameHallLogo:setScaleY(0.88)
end
...
function C:loadHeadBGAnimation()
	...
	self.node_headBg:addChild( skeletonNode )

    self.node_headBg:setScaleX(0.75)
    self.node_headBg:setScaleY(0.75)
end
