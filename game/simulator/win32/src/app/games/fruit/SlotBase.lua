...
--				self.m_arrSlots[i]:setPositionY(self.m_arrSlots[i]:getPositionY() - 5)
    ...
    self:setPositionX((display.width  - 1334 * display.height / 750) / 2)
        ...
        cc.CallFunc:create(function()
            if self.m_GameData.runInfo[nCol].accMoment then
                ...
                if self.m_GameData.runInfo[nCol].accEff then      -- 是否有加速特效
                    ...
                    local offset = self.m_GameData.runInfo[nCol].accEffOffset or cc.p(0, 0)
                    ...
                    eff:setPosition(cc.pAdd(cc.p(eff:getPosition()), offset))
