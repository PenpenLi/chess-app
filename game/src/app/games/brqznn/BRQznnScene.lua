...
-- 资源绑定
C.RESOURCE_BINDING = {
	topLeft_panel = {path="topLeft_panel"},
	topRight_panel= {path="topRight_panel"},
	topBg = {path="table_img"},
	--电池
	batteryNode = {path="topRight_panel.battery_node"},
	-- --电池充电标识
	-- batteryLighting = {path="top_panel.battery_img.lighting"},
	--房号文本
	roomIDLabel = {path="topLeft_panel.roomID"},
	--玩法文本
	roomTypeLabel = {path="topLeft_panel.roomType"},
	--底分文本
	roomDifenLabel = {path="topLeft_panel.roomDiFen"},
	--时间文本
	roomTimeLabel = {path="topRight_panel.time"},
	--局数文本
	roomTurnLabel = {path="topLeft_panel.roomTurn"},
	--菜单按钮
	menuBtn = {path="topRight_panel.menu_btn",events={{event="click",method="onClickMenuBtn"}}},
	--菜单界面
	menuPanel = {path="menu_panel",events={{event="click",method="onClickMenuBtn"}}},
	menuBG = {path="menu_panel.bg_btn",events={{event="click",method="onClickMenuBtn"}}},
	--帮助按钮
	helpBtn = {path="topLeft_panel.help_btn",events={{event="click",method="onClickHelpBtn"}}},
	--返回按钮
	...
	--摊牌按钮
	tanpaiBtn = {path="opt_panel.tanpai_btn",events={{event="touch",method="onClickTanpaiBtn"}}},
	--翻牌按钮
	flippaiBtn = {path="opt_panel.flippai_btn",events={{event="touch",method="onClickFlipBtn"}}},
	--搓牌按钮
	cuopaiBtn = {path="opt_panel.cuopai_btn",events={{event="touch",method="onClickCuoBtn"}}},
	--准备按钮
	zhunbeiBtn = {path="opt_panel.ready_btn",events={{event="touch",method="onClickReadyBtn"}}},
...
function C:ctor(core)
	...
	for i=1,8 do
		...
		local key1 = string.format("player%d_info_img",i)
...
function C:initialize()
	...
	self:adjustUI(self.topBg,{self.topLeft_panel},{self.menuPanel,self.topRight_panel})
	--zorder 1
	self.playerClassArr = {}
	for i=1,8 do
		local key = string.format("player%d",i)
		...
		-- end
		local key1 = string.format("player%d_info_img",i)
		local panel1 = self[key1]
		panel1:setTag(i)
	end
	--zorder 3
	self.topLeft_panel:setLocalZOrder(self.TOP_ZORDER)
	self.topRight_panel:setLocalZOrder(self.TOP_ZORDER)
	--zorder 4
	for i=1,4 do
	...
	--self:playFapaiSound(10)
end

--播放发牌声音
function C:playFapaiSound(times)
	local arr = {}
	arr[2]= cc.DelayTime:create(0.24)
	arr[1]= cc.CallFunc:create(function()
		PLAY_SOUND(self.model.soundPath.."fapai.mp3")
		PLAY_SOUND(self.model.soundPath.."fapai1.mp3")
		PLAY_SOUND(self.model.soundPath.."fapai2.mp3")
	end)
	self:runAction(CCRepeat:create(cc.Sequence:create(arr),times))
end

function C:onEnterTransitionFinish()
	C.super.onEnterTransitionFinish(self)
	--播放背景音乐
	PLAY_MUSIC(self.model.soundPath.."table_bgm.mp3")
end
...
function C:onExitTransitionStart()
	...
	utils:removeTimer(self.model.timerName..".playTypeSound")
	utils:removeTimer(self.model.timerName..".cleanDesktop")
	utils:removeTimer(self.model.timerName..".settlement")
	utils:removeTimer(self.model.timerName..".alertrecharge")
	--移除图集
...
function C:setDifen( text )
	...
	self.resultClass:setBeishu(text)
end
--设置局数
...
function C:setPlayerBlance( blance, playerId )
    --print("000000000000000000000000000000000000" .. playerId .. "------------" .. blance)
...
function C:showPlayerQiangTips( localSeatId, beishu, ani )
	printInfo(localSeatId..">>>>>>>>>>>>显示玩家抢庄倍数>>>>>>>>>>>>>"..beishu)
	local playerClass = self.playerClassArr[localSeatId]
...
--翻自己的牌面
function C:turnSelfPoker(count)
	local playerClass = self.playerClassArr[1]
	playerClass:turnPoker(count)
end
...
function C:doSettlement( info )
	local callback = function()
		...
		utils:delayInvoke(self.model.timerName..".cleanDesktop",5,function()
			self:cleanDesktop()
			self:cleanPlayers()
		end)
	end
