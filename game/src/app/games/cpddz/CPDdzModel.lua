...
function C:resetSoundIndex()
    self.sound_index=1
    printInfo(">>>>>>>>>>self.sound_index>>>>>>>>>>>>"..self.sound_index)
end

function C:playSendCardSound()
    printInfo(">>>>>>>>>>playSendCardSound>>>>>>>>>>>>"..self.sound_index)
    if self.sound_index>17 then
        self.sound_index=1
    end
    PLAY_SOUND(GAME_CPDDZ_SOUND_RES.."s_sendcard"..self.sound_index..".mp3")
    self.sound_index=self.sound_index+1
end
