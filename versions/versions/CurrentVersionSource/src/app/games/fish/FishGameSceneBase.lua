...
Fish2dTools.kRevolutionWidth = display.width / display.height * 750
...
--    display.setAutoScale(CC_DESIGN_RESOLUTION2)
    self:setScale(display.height / 750)
--    self:AddEvents(
--    -- 框架部分的用户状态通知
...
function FishGameSceneBase:Update(deltaT)
    ...
    rolledAnnounceLayer:setPositionX(Fish2dTools.kRevolutionWidth / 2)
...
--        App.showMsgBox(App.stringsDic.System_Tips_mb2, MsgBoxBtnType.NONEBUTTON)
        if self.dlg == nil then
            self.dlg = DialogLayer.new()
        end

        self.dlg:show("金币不足，无法发射子弹！", function() self.dlg = nil end)
...
function FishGameSceneBase:S_Catch_Bird(data)
    ...
    cannonPos = cannon:getParent():convertToNodeSpace(cannonPos)
...
function FishGameSceneBase:S_Send_Bullet(data)
    ...
    if self.m_bulletLayer:getChildrenCount() > 100 then
        return
    end

    local rot = data.rotation_
    rot = math.pi * 2 - rot + math.pi / 2
    if rot > math.pi * 2 then
        rot = rot - math.pi * 2
    end
    local localizedRot = math.atan(math.tan(rot) / Fish2dTools.mWorldScaleRate.x)
    if rot > math.pi * 1.5 then
        localizedRot = localizedRot + math.pi
    end
    if rot > math.pi / 2 then
        localizedRot = localizedRot + math.pi
    end
    localizedRot = math.pi * 2 - localizedRot - 3 * math.pi / 2
    if localizedRot < 0 then
        localizedRot = localizedRot + math.pi * 2
    end
    if localizedRot < math.pi / 2 then
        localizedRot = localizedRot + math.pi * 2
    end
    data.rotation_ = localizedRot
...
--        print("Waining toFire: m_BulletCount, BULLETCOUNT_MAX: ", self.m_BulletCount, Fish2dTools.BULLETCOUNT_MAX)
    ...
    local rot = myCannon:getBowRotation()
    rot = math.pi * 2 - rot + math.pi / 2
    if rot > math.pi * 2 then
        rot = rot - math.pi * 2
    end
    local normalizedRot = math.atan(math.tan(rot) * Fish2dTools.mWorldScaleRate.x)
    if rot > math.pi * 1.5 then
        normalizedRot = normalizedRot + math.pi
    end
    if rot > math.pi / 2 then
        normalizedRot = normalizedRot + math.pi
    end
    normalizedRot = math.pi * 2 - normalizedRot - 3 * math.pi / 2
    if normalizedRot < 0 then
        normalizedRot = normalizedRot + math.pi * 2
    end
    if normalizedRot < math.pi / 2 then
        normalizedRot = normalizedRot + math.pi * 2
    end

    local sendBulletData = { }
    sendBulletData.chair_id_ = self._dataModel.m_myChairId
    sendBulletData.rotation_ = normalizedRot
...
--    print("============>", myCannon:getBowRotation(), normalizedRot)
...
function FishGameSceneBase:netSendBullet(cmd_sendBullet, bMeLocalFire)
    ...
    self.m_cannonLayer:bulletSend(cmd_sendBullet.chair_id_, cmd_sendBullet.bullet_mulriple, cannon:isMe() and cannon:getBowRotation() or math.rad(cannonDegree), cmd_sendBullet.cur_gold_, bMeLocalFire)
    ...
    if isToParse then
        ...
        --cannon:setCannonClass(self.curCannonClass)
    ...
    for i = 0, bulletCount - 1 do
        if i == 0 then
            ...
            bulletStartPos = cannon:getParent():convertToNodeSpace(bulletStartPos)
        elseif i == 1 then
...
--    dump(cmd_send_fish)
    ...
    local worldScaleRate = self.m_fishLayer:getWorldScaleRate()
    cmd_send_fish.start_ = cmd_send_fish.start_ or cc.p(cmd_send_fish.start_x, cmd_send_fish.start_y)
    cmd_send_fish.pause_ = cmd_send_fish.pause_ or cc.p(cmd_send_fish.pause_x, cmd_send_fish.pause_y)
    cmd_send_fish.end_ = cmd_send_fish.end_ or cc.p(cmd_send_fish.end_x, cmd_send_fish.end_y)
    cmd_send_fish.start_ = cc.pMul(cmd_send_fish.start_, worldScaleRate)
...
function FishGameSceneBase:collisionPosFish(touchPos)
    local isTouch = false
    local pt_touchPos = self:convertToNodeSpace(touchPos)
...
function FishGameSceneBase:destoryFish(fishModel)
    ...
    --    end
--    print("DD: ", fishModel.node_)
--    fishModel.node_ = nil -- 这里nil后很多地方延迟处理还要访问这个，导致报错 WB
...
function FishGameSceneBase:Ding_play()
    ...
    armature:setScaleX(Fish2dTools.kRevolutionWidth / 1334)
    self.m_coinLayer:addChild(armature)
    self:fish_ding(true)
...
--    SoundMng:stopAllSounds()
    ...
    spr_background2_:setScaleX(Fish2dTools.mWorldScaleRate.x)
