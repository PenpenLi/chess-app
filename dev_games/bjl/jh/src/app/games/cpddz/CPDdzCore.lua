...
function C:onPlayerState(s)
    ...
    if localSeat == 1 and s.playerstate == self.define.playerGameState.Game then
        printInfo("_______________________1")
        self.model.isGaming = true
    else
        printInfo("_______________________2")
        self.model.isGaming = false
    end
end
...
function C:onToOtherRoom(s)
    ...
    if s.beilv then
        if jiaofen > 0 then
            self.scene:setBeiLv(s.beilv * jiaofen)
        else
            self.scene:setBeiLv(s.beilv)
        end
    end
    ...
    if s.gamestate == self.define.gameState.AddMuti then
        if s.jiabeiinfo then
            for i ,v in pairs(s.jiabeiinfo) do
                local localSeat = self:getLocalSeat(v.seat)
                if v.isjiabei == 1 then
                    self.scene:showJiaBei(localSeat)
                else
...
function C:onSettlement(s)
    ...
    info.rows = {}
    local difen = utils:moneyString(s.jiesuan.difen,2)
...
function C:s2cSetBeiLv(s)
    ...
    if self.model.jiaoFen > 0 then
        self.scene:setBeiLv(self.model.jiaoFen * s.beilv)
    else
        self.scene:setBeiLv(s.beilv)
    end
end
...
function C:c2sExchangeOut(cards)
    ...
    dump(pai,"_______________c2sExchangeOut_____________________")
...
function C:c2sExchangeIn(cards)
    ...
    dump(pai,"_______________c2sExchangeIn_____________________")
...
function C:c2sSelectDizhuCards(cards)
    ...
    dump(pai,"_______________c2sSelectDizhuCards_____________________")
...
function C:s2cExchangeOutCards(s)
    ...
    dump(s,"_______s2cExchangeOutCards____________  2133  ")
...
function C:s2cExchangeOutCardsFinish(s)
    ...
    dump(s,"_____________s2cExchangeOutCardsFinish_______________" .. tostring(os.date("%M:%S",os.time())))
    ...
    self:lockMsgForTime(3)
end

-- 通知一个玩家选进了牌
function C:s2cExchangeInCards(s)

    dump(s,"_______________s2cExchangeInCards________________________" .. tostring(tostring(os.date("%M:%S",os.time()))))
