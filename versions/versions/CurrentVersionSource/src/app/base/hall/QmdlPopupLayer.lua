...
C.RESOURCE_BINDING = {
    ...
    tgxx_btn_s = {path="box_img.tab_listview.tgxx_btn_s"},
    wdtd_btn_s = {path="box_img.tab_listview.wdtd_btn_s"},
    yjlq_btn_s = {path="box_img.tab_listview.yjlq_btn_s"},
    yjmx_btn_s = {path="box_img.tab_listview.yjmx_btn_s"},
}
...
function C:refreshQrcodeImg()
    ...
    print("dataManager.qrcodeUrl------******" .. dataManager.qrcodeUrl)
...
--                self:shareQrcodeUrl(imageUrl)
                ...
                print("333333333333333333333333333333333333333333")
                    ...
                    self:shareQrcodeUrl(dataManager.qrcodeUrl)
				end
			end)
		end
	else
...
function C:requestShareQrcodeImgPath(qrcodeUrl,callback)
	...
	local url = domain.."platform="..tostring(dataManager.styleId).."&url="..qrcodeUrl.."&sign="..tostring(sign).."&id="..tostring(dataManager.playerId)
...
function C:shareQrcodeImg( filePath )
    --print("分享filePath------" .. filePath)
	local info = {}
	info.title = "QQ娱乐"
	info.description = "精彩刺激好玩的游戏尽在QQ棋牌"
...
--分享二维码地址
function C:shareQrcodeUrl( urlText )
    local info = {}
    info.text = urlText
    print("info.text-------------" .. tostring(info.text))
	utils:shareToWechat(1,0,info)
end

--保存二维码
function C:saveQrcodeImg( filePath )
    --print("保存filePath------" .. filePath)
