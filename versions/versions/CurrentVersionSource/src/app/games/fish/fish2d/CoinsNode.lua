...
function CoinsNode:initNode()
    ...
    self.label_coin_ = ccui.TextBMFont:create("0", "games/fish/bmfonts/font_jinbishuzi.fnt")
...
function CoinsNode:show_coin(count,score)
	...
	self.mCoinBg:setPosition(cc.p(0, 5 * count + 15))
...
--    end
    showStr = utils:moneyString(score)
	self.label_coin_:setString(showStr)
	self.label_coin_:setVisible(false)
	self.label_coin_:setAnchorPoint(cc.p(0.5, 0.5))
	--< +的是字的高度
    self.label_coin_:stopAllActions()
	self.label_coin_:setPosition(cc.p(0, 5 * count - 13))
		...
		if (i == count - 1) then
			act = cc.Sequence:create(
				...
				cc.CallFunc:create(function ()
                    ...
                    self.label_coin_:runAction(cc.CallFunc:create(function() self.label_coin_:runAction(cc.RepeatForever:create(cc.JumpBy:create(2, cc.p(0, 0), 20, 4))) end))
--                    self.label_coin_:setRotation(-90)
