...
--	assert(count ~= 0);
	...
	for k = 1, count do
		...
		local sptFrame = cache:getSpriteFrame(str);
...
--    end
    ...
    local bg = ccui.ImageView:create("games/fish/loading/loadingBG.png")
    bg:setScale9Enabled(true)
    bg:setCapInsets(cc.rect(15, 320, 1, 1))
    bg:setPosition(cc.p(0, Fish2dTools.kRevolutionHeight))
    bg:setContentSize(cc.size(display.width * 750 / display.height, 750))
    self:addChild(bg, 99);

    local logo = display.newSprite("games/fish/loading/loading_logo.png")
    local loading = display.newSprite("games/fish/loading/loading_3.png")
    local barBG = display.newSprite("games/fish/loading/loading_1.png")
    local bar = ccui.LoadingBar:create("games/fish/loading/loading_2.png")
    bar:setPosition(barBG:getAnchorPointInPoints());
    logo:setPosition(cc.p(bg:getContentSize().width / 2, 450));
    loading:setPosition(cc.p(bg:getContentSize().width / 2, 160));
    barBG:setPosition(cc.p(bg:getContentSize().width / 2, 220));
    barBG:addChild(bar)
    bg:addChild(loading);
    bg:addChild(logo);
    bg:addChild(barBG);
    ...
    local update = function(dt)
        ...
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level2/bing_lizi.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level2/bing_quan.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level1/dajinbi.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level1/xiao1.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level1/xiao2.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level3/dabao_lizi.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level3/dabao_quan.plist", 1, 0)
        Fish2dTools.particle_play(self, cc.p(0, 0), Fish2dTools.mGameResPre .. "/flash/Particle/level4/lizi.plist", 1, 0)
...
--进入场景
function Fish2dGameScene:onEnterTransitionFinish()
	Fish2dGameScene.super.onEnterTransitionFinish(self)
	PLAY_MUSIC(GAME_FISH_SOUND_RES..string.format("bgm%d.mp3", math.random(1, 4)))
end

--退出场景
function Fish2dGameScene:onExitTransitionStart()
	STOP_MUSIC()
    self:finalize()
	Fish2dGameScene.super.onExitTransitionStart(self)
end


--更新玩家金币
function Fish2dGameScene:updatePlayerMoney( playerId, coin )
    self.m_cannonLayer:getCannon(self.model:getPlayer(playerId).seat):setGold(coin)
end
...
--        if self.sendQuit then
--            return self.core:quitGame()
--        end
--        self.core:sendQuitMsg()
--        self.sendQuit = true
        self.core:sendQuitMsg()
        self:ILeave(0, self.model.mySeatId)
    end
end
...
function Fish2dGameScene:UIToolBar() 
    ...
    Panel_Help:setPositionX(Panel_Help:getPositionX() * Fish2dTools.mWorldScaleRate.x)

    self.m_Var.m_Image_ExitTip = Image_ExitTip
    self.m_Var.m_Text_ExitTipCountdown = Image_ExitTip:getChildByName("AtlasLabel_Time")
    self.m_Var.m_Image_ExitTip:setPositionX(self.m_Var.m_Image_ExitTip:getPositionX() * Fish2dTools.mWorldScaleRate.x)

    -- 自动锁定按钮
    self.plockButton = Panel_ToolBar:seekChildByName("Button_suo")
    ...
    -- 自动射击按钮
    self.autoFireButton = Panel_ToolBar:seekChildByName("Button_Auto")
    ...
    self.autoFireButton:getParent():setPositionX(display.width * 750 / display.height)
...
--        local frameSize = glview:getFrameSize()
        local offsetX = GET_PHONE_HAIRE_WIDTH()
        if offsetX > 0 then
            offsetX = offsetX+20
        end
        buttonParent:setPositionX(buttonParent:getPositionX()+offsetX)
    end
    ...
    local helpClose = Panel_Help:seekChildByName("Button_Close")
...
function Fish2dGameScene:UIInitSetLockFish()
    ...
    Panel_SetLockFish:setPositionX(Panel_SetLockFish:getPositionX() * Fish2dTools.mWorldScaleRate.x)
    ...
    self.set_use_select:addClickEventListener( function()
        ...
        local fishSel = dataManager:getLastFishSetting()
        dataManager:setLastFishSetting(string.sub(fishSel, 1, 13).."11111")
    end )
    ...
    for i = 1, fishItemNode:getChildrenCount() do
        ...
        if fishItem then
            ...
            fishItem:addClickEventListener( function()
                ...
                local fishSel = dataManager:getLastFishSetting()
                fishSel = string.sub(fishSel, 1, i - 1)..(selectNode:isVisible() and "1" or "0")..string.sub(fishSel, i + 1, #fishSel)
                dataManager:setLastFishSetting(fishSel)
            end )
            local fishType = fishItem:getTag() + 12 - 1
            local fishSel = dataManager:getLastFishSetting()
            self.m_Var.m_canAutoLockFishType[fishType] = (string.byte(fishSel, i) == 49 and fishType or nil)
...
function Fish2dGameScene:setLockPanelAllFish(isLock)
    ...
    dataManager:setLastFishSetting(string.rep(isLock and "1" or "0", 18))
...
function Fish2dGameScene:lockAutoCallback(btnType, isOpen)
    local Panel_ToolBar = self:seekChild("Panel_ToolBar")
    local plockButton = Panel_ToolBar:seekChildByName("Button_suo")
    ...
    local autoFireButton = Panel_ToolBar:seekChildByName("Button_Auto")
...
function Fish2dGameScene:init_scene()
    ...
    self.m_Var.spr_background_:setScale(math.max(1, Fish2dTools.mWorldScaleRate.x))
    ...
    spr_water_wave_:setScale(1.31 * Fish2dTools.mWorldScaleRate.x, 1.46)
...
function Fish2dGameScene:playBossEnterEffect()
    ...
    if bossEnterDiban == nil then
        ...
        bossEnterDiban:setScaleX(Fish2dTools.mWorldScaleRate.x)
    ...
    if bossEnterDibanUp == nil then
        ...
        bossEnterDibanUp:setScale(-Fish2dTools.mWorldScaleRate.x)
