...
C.BINDING = {
    ...
    number1={path="number1"},
    number2={path="number2"},
    color1={path="color1"},
    color2={path="color2"},
}
...
function C:show(callback)
    ...
    self.number1:setOpacity(0)
    self.number2:setOpacity(0)
    ...
    local strBack = GAME_BRQZNN_IMAGES_RES..'poker2/'.."beimian_1.png"
    local strFront = GAME_BRQZNN_IMAGES_RES..'poker2/'.."card"..color.."_"..number..".png"
    self.rubCardLayer = RubCardLayer:create(strBack,strFront , display.cx, display.cy-55, endCallBack)
...
function C:createNum(frontImg, color,value)
     --数字
     self.number1:loadTexture(GAME_BRQZNN_IMAGES_RES..'poker2/'..self.numberNames[color+1]..value..'.png')
     self.number2:loadTexture(GAME_BRQZNN_IMAGES_RES..'poker2/'..self.numberNames[color+1]..value..'.png')
     self.color1:loadTexture(GAME_BRQZNN_IMAGES_RES..'poker2/'..self.colorNames[color+1]..'.png')
     self.color2:loadTexture(GAME_BRQZNN_IMAGES_RES..'poker2/'..self.colorNames[color+1]..'.png')
 end

--显示数字
function C:showNumber(val)
    local array = {}
    array[1]=cc.DelayTime:create(0.3)
    array[2]=cc.CallFunc:create(function()
        self.number1:runAction(cc.FadeIn:create(1.0))
        self.number2:runAction(cc.FadeIn:create(1.0))
        self.color1:runAction(cc.FadeIn:create(1.0))
        self.color2:runAction(cc.FadeIn:create(1.0))
     end)
    array[3]=cc.DelayTime:create(2)
...
function C:clean()
    ...
    self.number1:stopAllActions()
    self.number2:stopAllActions()
