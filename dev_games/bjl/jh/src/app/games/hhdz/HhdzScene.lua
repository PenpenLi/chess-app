...
C.RESOURCE_BINDING = {
	...
	-- --电池进度
	-- batteryBar = {path="top_panel.battery_img.bar"},
	-- --电池充电标识
    -- batteryLighting = {path="top_panel.battery_img.lighting"},
    --电池
    battery_node={path="top_panel.battery_node"},
	--帮助按钮
...
function C:initialize()
    ...
    --绑定电池节点
	self:bindBatteryNode(self.battery_node)
	self:updateBattery()
...
function C:createTrend()
    ...
    self.trendLayer:setPosition(cc.p( display.cx -TREND_LAYER_WIDTH / 2 + 2,display.top - 168))
...
function C:playerVsAnim(callBack)
	...
	black:setOpacity(0)
	black:setPosition(cc.p(display.cx -235,display.cy))
	black:addTo(node)

	local blackDelay1 = CCDelayTime:create(0.02)
	local blackFadeIn = CCFadeIn:create(0.06)
	local blackMoveIn = CCMoveTo:create(0.06, cc.p(display.cx-95, display.cy))
	local blackDelay2 = CCDelayTime:create(0.05)
	local blackMoveOut1 = CCMoveTo:create(0.05, cc.p(display.cx-135, display.cy))
	local blackDelay3 = CCDelayTime:create(1.0)
	local blackMoveOut2 = CCMoveTo:create(0.05, cc.p(display.cx-205, display.cy))
	...
	red:setOpacity(0)
	red:setPosition(cc.p(display.cx + 235,display.cy))
	red:addTo(node)

	local redDelay1 = CCDelayTime:create(0.02)
	local redFadeIn = CCFadeIn:create(0.06)
	local redMoveIn = CCMoveTo:create(0.06, cc.p(display.cx+95, display.cy))
	local redDelay2 = CCDelayTime:create(0.05)
	local redMoveOut1 = CCMoveTo:create(0.05, cc.p(display.cx+135, display.cy))
	local redDelay3 = CCDelayTime:create(1.0)
	local redMoveOut2 = CCMoveTo:create(0.05, cc.p(display.cx+205, display.cy))
	...
	king:setScale(0.6)
	king:setOpacity(0)
	king:setPosition(cc.p(display.cx-310,display.cy+60))
	king:addTo(node)

	local kingDelay1 = CCDelayTime:create(0.02)
	local kingFadeIn = CCFadeIn:create(0.06)
	local kingScaleIn = CCScaleTo:create(0.06, 1, 1)
	local kingMoveIn = CCMoveTo:create(0.06, cc.p(display.cx-180, display.cy+65))
	local kingMoveOut1 = CCMoveTo:create(0.08, cc.p(display.cx-210, display.cy+65))
	local kingDelay2 = CCDelayTime:create(1.02)
	local kingMoveOut2 = CCMoveTo:create(0.05, cc.p(display.cx-290, display.cy+65))
	...
	queen:setScale(0.6)
	queen:setOpacity(0)
	queen:setPosition(cc.p(display.cx + 325,display.cy + 60))
	queen:addTo(node)
	...
	local queenMoveIn = CCMoveTo:create(0.06, cc.p(display.cx+180, display.cy+70))
	local queenMoveOut1 = CCMoveTo:create(0.08, cc.p(display.cx+210, display.cy+70))
	local queenDelay2 = CCDelayTime:create(1.02)
	local queenMoveOut2 = CCMoveTo:create(0.05, cc.p(display.cx+300, display.cy+70))
    ...
    anim_light:setPosition(cc.p(display.cx,display.cy))
    ...
    vsIcon:setScale(0)
    vsIcon:setOpacity(0)
    vsIcon:setPosition(cc.p(display.cx,display.cy))
    vsIcon:addTo(node)
    ...
    local vs_part2_move1 = CCMoveTo:create(0.03, cc.p(display.cx+-45,display.cy+30))
    ...
    local vs_part3_move1 = CCMoveTo:create(0.03, cc.p(display.cx,display.cy))
    local vs_part3_move2 = CCMoveTo:create(0.03, cc.p(display.cx+45,display.cy+30))
    local vs_part3_move3 = CCMoveTo:create(0.03, cc.p(display.cx,display.cy))
    ...
    local nodeCallFun1 = CCCallFunc:create(function (  )
    	...
    	node.starTail:setPosition(cc.p(display.cx,display.cy))
    	...
    	starBomb:setPosition(cc.p(display.cx,display.cy))
    	starBomb:addTo(node)
