...
function C:init()
    ...
    if self.pView_ then
        ...
        if self.pos_ == JsmjDefine.MahjongPos.POSITION_TOP then
            tileHeight = 32  --此处可以据牌张大小进行调整,可以调整牌间距
...
--        array[1] = CCMoveTo:create(0.15, cc.p(x, y))
--        array[2] = CCCallFunc:create(function()
--            --self.pView_:askCleanHoldMsgForBit(self.pView_.HOLD_BIT_OTHER_DRAW)
--        end)
        img:runAction( CCMoveTo:create(0.15, cc.p(x, y)) )
    end
end
...
function C:drawTopTiles(winFlag)
   ...
   --     self.currentNum_, ",self.isAnim_=", self.isAnim_)
    ...
    elseif self.isAnim_ then
        local drawed = startIndex
        if self.currentNum_ == self.mahjongData_:getMaxTileNumber()  then
            if self.isAnGang_ == false then
                self:drawTile(MAX_TILE, true)
                drawed = startIndex + 1
            end
        end
         for i = drawed, MAX_TILE - 1 do
            ...
            if drawFlag == true and not self.isDrawFlower_ then
                ...
                if self.moveAnimFlag_ and finalLocation and (self.drawPlayerTile_ or self.currentNum_ % 3 == 2)
                    ...
                    if finalShowInfo and finalShowInfo.x and finalShowInfo.y then
                        ...
                        array:addObject(CCMoveTo:create(0.4, cc.p(finalShowInfo.x, finalShowInfo.y)))
...
function C:getLastShowLocation(pos)
    ...
    if showInfo and showInfo.x and showInfo.y then
        return cc.p(showInfo.x, showInfo.y)
    else
    end
end
...
function _getArrowInfo(selfPos, tilePos, width, height)
    local rotate, skewX, shewY, left, top, scaleX, scaleY = 0, 0, 0, 0, 0, 1, 1
    if selfPos == JsmjDefine.MahjongPos.POSITION_LEFT then
        left, top = width * 1 / 4, height * 3 / 4
        if tilePos == JsmjDefine.MahjongPos.POSITION_RIGHT then
            skewX, shewY = 20, 0
        elseif tilePos == JsmjDefine.MahjongPos.POSITION_TOP then
            skewX, shewY = 10, -30
            rotate = -90
            scaleX = 0.8
        elseif tilePos == JsmjDefine.MahjongPos.POSITION_BOTTOM then
            skewX, shewY = 10, -20
            rotate = 90
            scaleX = 0.8
        end
    elseif selfPos == JsmjDefine.MahjongPos.POSITION_RIGHT then
        left, top = width * 3 / 4, height * 3 / 4
        if tilePos == JsmjDefine.MahjongPos.POSITION_LEFT then
            rotate = 180
            skewX, shewY = -20, 0
        elseif tilePos == JsmjDefine.MahjongPos.POSITION_TOP then
            skewX, shewY = 10, 10
    ...
    elseif selfPos == JsmjDefine.MahjongPos.POSITION_TOP then
        left, top = width / 2, height * 2 / 10
        scaleX = 0.8
        scaleY = 0.8
        if tilePos == JsmjDefine.MahjongPos.POSITION_RIGHT then
            skewX, shewY = 0, 0
        elseif tilePos == JsmjDefine.MahjongPos.POSITION_LEFT then
            rotate = 180
        elseif tilePos == JsmjDefine.MahjongPos.POSITION_BOTTOM then
            rotate = 90
        end
...
function C:getJiafanCountInHand()
    ...
    if self.mahjongData_ then
        ...
        if fanTile then
            local top = self.mahjongData_:getMahjongPlayerInfoByPos(JsmjDefine.MahjongPos.POSITION_TOP)
