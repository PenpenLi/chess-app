...
function C:ctor(parent)
    ...
    self.tileMargin = 0
    self.bottomOffset = math.modf(self.dimens_:getDimens(21))
    -- self.bottom = math.modf(self.dimens_:getDimens(32))
    self.bottom_ = math.modf(self.dimens_:getDimens(35))
    -- 距离底部的距离

    self.padding_ = math.modf(self.dimens_:getDimens(15))
    ...
    self.resultPadding_ = self.dimens_:getDimens(80)
    -- 结算时，　牌放倒的偏移
    self.paddingCenter_ = self.dimens_:getDimens(0)
    ...
    self.nTilePressEffectWidth_ = math.modf(self.dimens_:getDimens(106))
    self.nTilePressEffectHeight_ = math.modf(self.dimens_:getDimens(153))
    -- 吃碰杠几组牌
...
function C:playViewReset()
    ...
    -- 以下用于自动适配手牌张数
    self.bigTileWidth_ = math.modf(self.dimens_:getDimens(80))
    self.bigTileHeight_ = math.modf(self.dimens_:getDimens(116))
    local maxDealCardNum = 5
    self.tileAllWidth_ = maxDealCardNum * self.bigTileWidth_ +(maxDealCardNum - 1) * self.tileMargin
    self.margin_ = math.modf((self.dimens_.width - self.tileAllWidth_ - self.padding_) / 2)
    self.handStartX_ = self.dimens_.width - self.margin_ - self.bigTileWidth_+ self.dimens_:getDimens(45)
    -- 手牌的起始位置
...
--    end
    ...
    if num and not self.isShowTiles_ then
        ...
        if num == 4 then --前4张
            ...
            last = self:getHadShowNum()
            self:setHadShowNum(num)
        end
        --_debugInfo("showDealTiles began=", began, ",last=", last)
...
function C:drawBottomTilesAnim(count)
    ...
    if layer then
        ...
        -- 发牌动画的位置
        local x = self.handStartX_ + self.dimens_:getDimens(0)

        local maxDealCardNum = self.mahjongData_:getMaxTileNumber()
        if count == maxDealCardNum then
            x = x - self.padding_ - self.bigTileWidth_ - self.tileMargin
        end
        --_debugInfo(" drawBottomTilesAnim maxDealCardNum=", maxDealCardNum, ",x=", x, ",count=", count)
...
--            print(debug.traceback())
            ...
            tileView:setScale(self.dimens_.scale_)
            tileView:setVisible(false)
            tileView:setAnchorPoint(cc.p(0, 0))
            self:addChild(tileView, i * 2 - 1, i)
            tileView:setPosition(cc.p(x, self.bottom_))
            x = x - self.bigTileWidth_ - self.tileMargin
            if i == (maxDealCardNum + 1) then
                x = x - self.padding_
            end
            --_debugInfo("drawBottomTilesAnim i= ", i, ",x=", x, ",count=", count)
        end
    end
end
...
function C:bottomTilesAnim()
    ...
    if layer and self.openDoorStepState_ <= STATE_OPENDOOR_STEP_2 then
        if self.openDoorStepState_ == STATE_OPENDOOR_STEP_1 then
            --_debugInfo(" --bottomTilesAnim case 1 #self.tiles_=", #self.tiles_)
            for i = 1, #self.tiles_, 1 do
                layer:setVisible(true)
                local view = layer:getChildByTag(i)
                --_debugInfo(" --bottomTilesAnim i=", i, ",tileId=", self.tiles_[i].id_, ",view=", view)
                if view then
        ...
        elseif self.openDoorStepState_ == STATE_OPENDOOR_STEP_2 then
            --_debugInfo(" --bottomTilesAnim case 2")
        ...
        --_debugInfo(" --bottomTilesAnim case 3")
        self:stopSchedule()
        self:setTouchEnabled(true)
        self:setOpenDoorEndFlag(true)
        if self.parentView.isLastTable_ then
           -- _debugInfo("----bottomTilesAnim()----600---[isLastTable => playChampionAnim]---")
        end
    end
end
...
--    if self.colorCount_ == nil then
--        self.colorCount_ = {}
--        self.colorCount_[0] = 0
--        self.colorCount_[1] = 0
--        self.colorCount_[2] = 0

--        for k, v in ipairs(self.tiles_) do
--            if v and v.color_ <= 2 then
--                self.colorCount_[v.color_] = self.colorCount_[v.color_] + 1
--            end
--        end
--    end
    table.sort(self.tiles_, function(a, b)
--        if a.color_ ~= nil then
--            if a.color_ == b.color_ then
...
--                end
--            else
--                if a.color_ <= 2 and b.color_ <= 2 then
--                    if self.colorCount_[a.color_] == self.colorCount_[b.color_] then
--                        return a.color_ < b.color_
--                    else
--                        return self.colorCount_[a.color_] > self.colorCount_[b.color_]
--                    end
--                else
--                    return a.color_ < b.color_
--                end
...
function C:addTileViews()
    ...
    for i = count, 1, -1 do
        ...
        if not tileView then
            tileView = tile:getTileImage(self.dimens_.scale_, nil, self.bigTileWidth_, self.bigTileHeight_)
...
--            end)
    ...
    for i = count, 1, -1 do
        ...
        if tile ~= nil then
            local tileView = self:getChildByTag(tile.id_)
            if count % 3 == 2 and self.mahjongData_.tingInfo then
                -- 5张牌走此流程
...
function C:drawTiles(notAnim)
    if not (self:getOwnState() >= STATE_SORT_ANIM) then
        print(" drawTiles state error and return ")
        return
    end
    local layer = self:getChildByTag(ADD_ID_ANIM)
    if layer then
    ...
    if self.tiles_ then
        local count = #self.tiles_
        self.startX = self.handStartX_ + self.dimens_:getDimens(0)

        --if self.nGroup_ > 0 then
            print(" drawTiles case 1")
    ...
    if tileView then
        ...
        if self:getOpenDoorEndFlag() == true and not notAnim then
            if tile:getIsInHand() == false and drawtile == nil and i ~= #self.tiles_ then
                ...
                local array = { }
                array[1] = CCMoveTo:create(0.1, ccp(tile.tileX_, tile.tileY_ + self.bigTileHeight_)) -- 将牌先抬起
                array[2] = CCMoveTo:create(0.2, ccp(x, bottom + self.bigTileHeight_)) --横向移动
                array[3] = CCMoveTo:create(0.2, ccp(x, self.bottom_)) -- 落到底下
            ...
            elseif drawtile ~= nil and tile:getIsInHand() ~= true and drawtile.id_ == tile.id_ then
                ...
                if math.abs(tile.tileX_ - x) > self.bigTileWidth_ * 2 then
                    ...
                    local array = { }
                    table.insert(array,CCMoveTo:create(0.1, ccp(tile.tileX_, tile.tileY_ + self.bigTileHeight_)))
                    table.insert(array,CCMoveTo:create(0.2, ccp(x, bottom + self.bigTileHeight_))) --横向移动
                    table.insert(array,CCMoveTo:create(0.2, ccp(x, self.bottom_))) -- 落到底下
                    table.insert(array,CCCallFunc:create(function()
                        self:draweff(tileView, tile, x, self.bottom_, isLast)
                    ...
                    local moveto = CCMoveTo:create(0.2, ccp(x, self.bottom_))
...
function C:drawEffect(tile)
    ...
    if not effectView then
        local tileTop = (self.nTilePressEffectHeight_ - self.bigTileHeight_) / 2 + self.dimens_:getDimens(2.5)
        local tileLeft = (self.nTilePressEffectWidth_ - self.bigTileWidth_) / 2 - self.dimens_:getDimens(1)
        ...
        tileSelView:setScale(self.dimens_.scale_)
        tileSelView:setPosition(0, 0)
        touchLayer:addChild(tileSelView)

        local tileView = tile:getTileImage(self.dimens_.scale_)
        tileView:setAnchorPoint(cc.p(0, 0))
        ...
        if tileView then
            if self:getShowWinTiles() and not self:getTileCanTingFlag(tile) then
                ...
                if tileSelView then
                    ...
                    tileSelView = ccui.ImageView:create(GAME_JSMJ_IMAGES_RES .."game/tile/mahjong_tile_undisable.png")
                    tileSelView:setAnchorPoint(cc.p(0, 0))
                    tileSelView:setScale(self.dimens_.scale_)
                    tileSelView:setTag(IMG_TING_MASK_ID)
                    tileView:addChild(tileSelView)
                end
...
function C:drawLastResultTile(lastTile, needReMargin)
    local tile
    if lastTile then
        tile = lastTile
    else
    ...
    if tile and not self.resultView_ then
        ...
        self.resultView_:setContentSize(cc.size(self.dimens_:getDimens(70), self.dimens_:getDimens(102)))
        self.resultView_:setAnchorPoint(cc.p(0, 0))
        self.resultView_:setPosition(x, self.bottom_)
        self:addChild(self.resultView_)

        local lastTileView = tile:getSmallTileImage(self.dimens_.scale_)
        ...
        if lastTileView then
            ...
            resultTileMask:setScale(self.dimens_.scale_ * 0.8)
...
function C:setLastPoint(isShow, winFlag)
    local x = self.startX
    if isShow then
        local width = math.modf(self.dimens_:getDimens(70))
        x = self.dimens_.width - self.margin_ - 2.5 * width + self.resultPadding_
    else
        x = self.handStartX_
        --if self.nGroup_ > 0 then
        --    x = x - (self.dimens_:getDimens(18) + (self.nGroup_ - 1) * self.dimens_:getDimens(25))
    ...
    if self.resultView_ then
        --if winFlag and self.nGroup_ < 3 then
            local array = { }
            array[1] = CCMoveTo:create(0.2, ccp(x, self.bottom_))
...
function C:drawResultTile(winFlag, lastTile)
    local width = math.modf(self.dimens_:getDimens(70))
    local x = self.dimens_.width - self.margin_ - 3.5 * width - self.padding_ + self.resultPadding_
    --if self.nGroup_ > 0 then
    ...
    for i = count, 1, -1 do
        ...
        if count % 3 == 2 then
            ...
            if i == count then
                if not winFlag then
                    resultView = tile:getSmallTileImage(self.dimens_.scale_)
                    local x1 = x + math.modf(self.dimens_:getDimens(80))
    ...
    if lastTile then
        local needReMargin = true
        self:drawLastResultTile(lastTile, needReMargin)
    end
        ...
        if #self.tiles_ % 3 ~= 2 or #self.tiles_ > (maxTileNum + 1) then
            ...
            if self.mahjongData_.myOutCard and self.mahjongData_:getTiles() then
                ...
                for k, v in ipairs(tiles) do
                    if v ~= nil then
                        local tile = Tile.new(v.id_)
                        tile:setIsInHand(true)
                        table.insert(self.tiles_, tile)
                    end
                end
                self.drawTile_ = Tile.new(self.mahjongData_.myOutCard)
                table.insert(self.tiles_, Tile.new(self.mahjongData_.myOutCard))
                self:setDiscardTileFlag(true)
...
--            _debugInfo(" first tile id=", firstTile.id_)
            table.insert(self.tiles_, Tile.new(firstTile.id_))
        end
--        self:setGroupCount()
        -- 更新吃碰杠了几手牌
        self:addTileViews()
...
function C:refreshCanTingTiles()
    self.nCanTingTiles_ = {}
    local mahjongData = GameDataContainer:getGameData(self.matchId_)
    local tingResult = mahjongData:getTingResult()
    if mahjongData and tingResult and #tingResult > 0 then
        for k, v in ipairs(tingResult) do
            local discardColor = v.nDiscardColor_
            local discardValue = v.nDiscardValue_
            if (discardColor ~= nil and discardColor > -1) and (discardValue ~= nil and discardValue > -1) then
                table.insert(self.nCanTingTiles_, Tile.new(discardColor, discardValue))
                -- isTing = true
            end
        end
    end
...
function C:getTileCanTingFlag(owntile)
    for k, v in pairs(self.nCanTingTiles_) do
        if v.color_ == owntile.color_ and v.value_ == owntile.value_ then
            return true
        end
    end
    return false
...
function C:tingSignSelect()
    ...
    for i = count, 1, -1 do
        ...
        if tile ~= nil then
            ...
            if not self.mIsCalled_ and #self.tiles_ % 3 == 2 and self:getTileCanTingFlag(tile)
                    and (self.tingSignColor_ ~= tile.color_ or self.tingSignValue_ ~= tile.value_) then
...
function C:playChampionAnim(isRepeat)
    ...
    if count > 0 then
        ...
        animLayer:setAnchorPoint(ccp(0, 0))
        animLayer:setPosition(self.tiles_[1].tileX_, self.dimens_:getDimens(40))
        animLayer:setViewSize(touchWidth, self.dimens_:getDimens(103))
        ...
        championBg:setAnchorPoint(ccp(0, 0))
        championBg:setPosition(ccp(0, 0))
        championBg:setScale(self.dimens_.scale_)
        local array =  { }
        array[1] = CCMoveTo:create(1, ccp(touchWidth, 0))
        ...
        for i = 1, 5 do
            ...
            if resultView then
                ...
                if imgMask then
                    ...
                    if tileOwn.color_ == tile.color_ and tileOwn.value_ == tile.value_ then
